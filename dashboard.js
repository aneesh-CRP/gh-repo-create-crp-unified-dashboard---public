...
function renderSpeedToContact() {
  ...
  FB_CRM_DATA.forEach(function(row) {
    ...
    var crmDate = _parseDate(crmDateStr);
    if (!crmDate) return;
    ...
    if (ref && ref.date_created) {
      var refDate = _parseDate(ref.date_created);
      if (refDate) {
...
function renderRecruiterAttribution() {
  ...
  META_ADS_DATA.forEach(function(c) {
    ...
    // Find referrals matching these studies (deduplicate to prevent double-counting)
    var refs = [], refsSeen = new Set(), coordSet = new Set();
    studies.forEach(function(sk) {
      var skLower = sk.toLowerCase();
      REFERRAL_DATA.forEach(function(r) {
        if (refsSeen.has(r.id || r.name)) return;
        var rs = (r.study || '').toLowerCase().trim();
        if (rs && (rs.includes(skLower) || skLower.includes(rs))) { refs.push(r); refsSeen.add(r.id || r.name); }
      });
  ...
  rows.forEach(function(r) {
    ...
    var gapPct = r.metaLeads > 0 ? Math.min(100, Math.round((r.pipelineTotal / r.metaLeads) * 100)) : 0;
    html += '<tr style="border-top:1px solid #f1f5f9;">';
