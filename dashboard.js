...
function injectScheduleMedRecords() {
  ...
  rows.forEach(row => {
    ...
    const patCell = cells[3];
    const patName = (patCell.textContent || '').trim().toLowerCase();
    if (!patName) return;
    ...
    let badge = '';
    if (recDone && relDone) {
      badge = '<span style="display:inline-block;margin-left:6px;font-size:8px;font-weight:700;padding:1px 5px;border-radius:3px;background:#05966920;color:#059669;vertical-align:middle" title="Release: ' + escapeHTML(mr.medical_release||'') + ' | Records: ' + escapeHTML(mr.records_received||'') + '">RECORDS \u2713</span>';
    } else if (recDone) {
      badge = '<span style="display:inline-block;margin-left:6px;font-size:8px;font-weight:700;padding:1px 5px;border-radius:3px;background:#05966920;color:#059669;vertical-align:middle" title="Release: ' + escapeHTML(mr.medical_release||'') + ' | Records: ' + escapeHTML(mr.records_received||'') + '">REC \u2713</span>';
    } else if (recPending) {
      badge = '<span style="display:inline-block;margin-left:6px;font-size:8px;font-weight:700;padding:1px 5px;border-radius:3px;background:#d9770620;color:#d97706;vertical-align:middle" title="Release: ' + escapeHTML(mr.medical_release||'') + ' | Records: ' + escapeHTML(mr.records_received||'') + '">REC \u23f3</span>';
    } else {
      badge = '<span style="display:inline-block;margin-left:6px;font-size:8px;font-weight:700;padding:1px 5px;border-radius:3px;background:#dc262620;color:#dc2626;vertical-align:middle" title="Release: ' + escapeHTML(mr.medical_release||'No') + ' | Records: ' + escapeHTML(mr.records_received||'None') + '">REC \u2717</span>';
    }
    patCell.insertAdjacentHTML('beforeend', badge);
