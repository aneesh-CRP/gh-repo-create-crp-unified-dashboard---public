...
function renderRevenuePerUser() {
  ...
  function buildUserTable(data, labelBadgeId, label) {
    ...
    rows.forEach(function(e) {
      ...
      h += '<td style="text-align:center;color:#8b5cf6;">'+d.questions.toLocaleString()+'</td>';
...
function buildRiskFlagCards() {
  ...
  if (critical.length > 0 || high.length > 0) {
    ...
    html += `<span style="font-size:10px;font-weight:600;padding:3px 10px;border-radius:4px;background:#fffbeb;color:#d97706;border:1px solid #fde68a;">${flags.length - critical.length - high.length} MEDIUM (2×)</span>`;
    html += `</div>`;
  }
...
function renderCoordWorkloadBalance() {
  ...
  // Cancels from last 60 days for cancel rate
  ...
  var coordStats = COORDS.map(function(name) {
    ...
    var phl = 0, pnj = 0;
    var totalDuration = 0;
    visits.forEach(function(v) { patients[v.patient]=1; studies[v.study]=1; if ((v.site||'').indexOf('Penn')!==-1) pnj++; else phl++; totalDuration += (v.duration_min||0); });
    var cancelRate = (visits.length+cancels.length) > 0 ? Math.round(cancels.length/(visits.length+cancels.length)*100) : 0;
    var durationHrs = Math.round(totalDuration / 60 * 10) / 10;
    return { name:name, visits:visits.length, cancels:cancels.length, patients:Object.keys(patients).length, studies:Object.keys(studies).length, cancelRate:cancelRate, phl:phl, pnj:pnj, totalDuration:totalDuration, durationHrs:durationHrs };
  }).sort(function(a,b){return b.visits-a.visits;});

  var maxVisits = Math.max.apply(null, coordStats.map(function(c){return c.visits;}).concat([0])) || 1;
  var maxDuration = Math.max.apply(null, coordStats.map(function(c){return c.durationHrs;}).concat([0])) || 1;

  // Track current toggle mode
  var curMode = (window._coordBalanceMode || 'visits');

  var html = '<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:8px;">';
  html += '<div style="font-size:11px;color:#64748b;font-weight:600;">Upcoming Visit Distribution (next 30 days)</div>';
  html += '<div style="display:flex;gap:2px;background:#f1f5f9;border-radius:6px;padding:2px;">';
  html += '<button onclick="window._coordBalanceMode=\'visits\';renderCoordWorkloadBalance()" style="font-size:10px;font-weight:600;padding:3px 10px;border:none;border-radius:4px;cursor:pointer;background:'+(curMode==='visits'?'#fff':'transparent')+';color:'+(curMode==='visits'?'#1843AD':'#64748b')+';box-shadow:'+(curMode==='visits'?'0 1px 2px rgba(0,0,0,.1)':'none')+'">Visits</button>';
  html += '<button onclick="window._coordBalanceMode=\'duration\';renderCoordWorkloadBalance()" style="font-size:10px;font-weight:600;padding:3px 10px;border:none;border-radius:4px;cursor:pointer;background:'+(curMode==='duration'?'#fff':'transparent')+';color:'+(curMode==='duration'?'#1843AD':'#64748b')+';box-shadow:'+(curMode==='duration'?'0 1px 2px rgba(0,0,0,.1)':'none')+'">Duration</button>';
  html += '</div></div>';

  // Workload table
  var isDuration = curMode === 'duration';
  var barMax = isDuration ? maxDuration : maxVisits;
  html += '<table style="width:100%;border-collapse:collapse;font-size:12px;">';
  html += '<tr style="background:#f8fafc;"><th style="text-align:left;padding:6px 8px;font-weight:600;color:#64748b;font-size:10px;">Coordinator</th><th style="text-align:left;padding:6px 8px;font-weight:600;color:#64748b;font-size:10px;">'+(isDuration?'Total Hours':'Upcoming Visits')+'</th><th style="text-align:center;padding:6px 8px;font-weight:600;color:#64748b;font-size:10px;">Site</th><th style="text-align:center;padding:6px 8px;font-weight:600;color:#64748b;font-size:10px;">Patients</th><th style="text-align:center;padding:6px 8px;font-weight:600;color:#64748b;font-size:10px;">Studies</th></tr>';
  coordStats.forEach(function(c) {
    var barVal = isDuration ? c.durationHrs : c.visits;
    var barLabel = isDuration ? c.durationHrs + 'h' : '' + c.visits;
    var pct = barMax > 0 ? Math.round(barVal / barMax * 100) : 0;
    ...
    html += '<td style="padding:8px;"><div style="font-weight:700;color:#1e293b;">'+escapeHTML(c.name.split(' ')[0])+'</div></td>';
    ...
    html += '<div style="position:absolute;left:8px;top:0;font-size:11px;font-weight:700;color:'+(pct>30?'#fff':'#1e293b')+';line-height:18px;">'+barLabel+'</div>';
    html += '</div></div></td>';
