...
// ═══════════════════════════════════════════════════
let QB_DATA = { pnlMonthly: [], pnlByStudy: {}, pnlUnmatched: [], qbTotalIncome: 0, qbPeriod: '', invoiceLines: [], pnlClass: [], timeActivity: [], payments: [], empMap: {}, studyHoursAll: {}, studyBillableAll: {}, totalPayroll: 0, totalContractors: 0, totalTrackedHours: 0, totalBillableHours: 0, contractorNames: new Set(), contractorCosts: {}, expenseLines: [], loaded: false };
...
async function fetchQuickBooksData() {
  ...
  try {
    ...
    timeAct.filter(r => (r['Customer/Study']||'').trim()).forEach(r => {
      ...
      if (!empMap[emp]) empMap[emp] = { hours: 0, billableHrs: 0, amount: 0, studyHours: {}, studyBillable: {}, studyAmount: {}, entries: 0 };
    ...
    // ── Extract payroll, contractor totals, and named contractor costs from PnL expense section
    let totalPayroll = 0, totalContractors = 0;
    const contractorCosts = {}; // name -> cost (for named contractors in PnL)
    const expenseLines = []; // all expense accounts for reference
    let inExp = false;
    pnlMonthly.forEach(r => {
      const acct = (r['Account'] || Object.values(r)[0] || '').trim();
      if (!acct) return;
      ...
      if ((lo.includes('contractor') || lo.includes('subcontract')) && !lo.startsWith('total ')) {
        totalContractors += tot;
        // Extract named contractor: "Contractor - Name" or "Subcontractor - Name"
        const parts = acct.split(/[-–—]/);
        if (parts.length >= 2) {
          const cName = parts.slice(1).join('-').trim();
          if (cName) contractorCosts[cName] = (contractorCosts[cName]||0) + tot;
        }
      }
    });

    // Detect which employees are contractors by matching against PnL contractor names
    const contractorNames = new Set();
    Object.keys(contractorCosts).forEach(cn => {
      const cnLo = cn.toLowerCase().replace(/[\s\-]/g, '');
      Object.keys(empMap).forEach(emp => {
        const empLo = emp.toLowerCase().replace(/[\s\-]/g, '');
        if (cnLo.includes(empLo) || empLo.includes(cnLo) ||
            cnLo.split(/\s+/).some(w => w.length > 3 && empLo.includes(w))) {
          contractorNames.add(emp);
          empMap[emp].contractorCost = contractorCosts[cn];
        }
      });
    });
    ...
    QB_DATA.contractorCosts = contractorCosts;
...
function renderStaffEconomics() {
  ...
  const totalBillableHours = QB_DATA.totalBillableHours;
