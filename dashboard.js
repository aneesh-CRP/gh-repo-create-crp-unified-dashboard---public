...
function renderStaffEconomics() {
  ...
  const allRows = Object.entries(empMap).map(([name, data]) => {
    ...
    // Cost: PnL actual if found, else payroll blended rate × hours
    const estCost = pnlCost > 0 ? pnlCost : payrollRate * data.hours;
    const costSource = pnlCost > 0 ? 'pnl' : 'payroll';
    const roi = estCost > 0 ? attrRevenue / estCost : 0;
    const revPerHr = data.hours > 0 ? attrRevenue / data.hours : 0;

    return { name, role, hours: data.hours, numStudies: studies.length, pnlCost, attrRevenue, estCost, costSource, roi, revPerHr, studyDetail };
  });
  ...
  invList.forEach(inv => {
    ...
    let pnlCost = 0, matchedKey = '';
    Object.entries(pnlCostByName).forEach(([key, cost]) => {
      const kLo = key.toLowerCase();
      if (kLo.includes(inv.parts[0]) && kLo.includes(inv.parts[inv.parts.length-1])) { pnlCost = cost; matchedKey = key; }
    });
    allRows.push({
      name: inv.full + (pnlCost > 0 ? '' : ' (no QB data)'),
      role: 'investigator', hours: 0, numStudies: 0, pnlCost,
      attrRevenue: 0, estCost: pnlCost, costSource: pnlCost > 0 ? 'pnl' : 'none',
  ...
  function renderGroupTable(bodyId, rows, groupTotals, prefix) {
    ...
    tbody.innerHTML = rows.map(r => {
      ...
      const costLabel = r.costSource === 'pnl' ? '<div style="font-size:9px;color:var(--muted)">PnL actual</div>' : r.costSource === 'none' ? '<div style="font-size:9px;color:var(--muted)">no data</div>' : '';
      ...
      return '<tr style="cursor:pointer' + (noTime ? ';opacity:0.6' : '') + '" onclick="var d=document.getElementById(\'' + rid + '\');d.style.display=d.style.display===\'none\'?\'contents\':\'none\'">' +
        ...
        '<td class="r" style="color:#EF4444">' + (r.estCost > 0 ? fmtD(r.estCost) : '—') + costLabel + '</td>' +
