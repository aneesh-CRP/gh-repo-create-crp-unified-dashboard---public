...
async function loadRevenuePerUser(days, btn) {
  ...
  try {
    var url = (CRP_CONFIG.USE_CLOUD_FUNCTION && CRP_CONFIG.CF_BASE) ? CRP_CONFIG.CF_BASE + '?feed=revenuePerUser&format=json&days=' + _rpuDays : null;
    if (!url) { if (badge) badge.textContent = 'No cloud function'; return; }
...
function renderRevenuePerUser() {
  ...
  function buildUserTable(data, labelBadgeId, label) {
    var rows = Object.entries(data).filter(function(e){return e[0]!=='Unassigned';}).sort(function(a,b){return b[1].revenue-a[1].revenue;});
    ...
    h += '<th style="text-align:center;">eSource Q</th>';
    h += '<th style="text-align:left;">Top Study</th>';
    h += '<th style="text-align:center;min-width:120px;">Revenue</th>';
    h += '</tr></thead><tbody>';
    ...
    if (unassigned && unassigned.visits > 0) {
      h += '<tr style="border-bottom:1px solid #f1f5f9;color:#94a3b8;">';
      h += '<td style="padding:7px 12px;font-style:italic;">Unassigned (no eSource)</td>';
      h += '<td style="text-align:center;">'+unassigned.visits+'</td>';
      h += '<td style="text-align:center;">$'+Math.round(unassigned.revenue).toLocaleString()+'</td>';
      h += '<td colspan="4"></td></tr>';
    }
    // Total
    var totRev = Object.values(data).reduce(function(s,d){return s+d.revenue;},0);
    var totVis = Object.values(data).reduce(function(s,d){return s+d.visits;},0);
  ...
  if (coordEl) coordEl.innerHTML = buildUserTable(byCoord, 'rpu-coord-badge', 'Coordinator');

  // Render investigator table
  var invEl = el('rpu-inv-table');
  if (invEl) invEl.innerHTML = buildUserTable(byInv, 'rpu-inv-badge', 'Investigator');

  // Render recruiter table
  var recEl = el('rpu-rec-table');
  if (recEl) recEl.innerHTML = buildUserTable(byRecruiter, 'rpu-rec-badge', 'Recruiter');
...
function showRegPerfDetail(userName, metric) {
  ...
  (window._regPerfData || []).forEach(function(r) {
    ...
    s.pending_duties += parseInt(r.pending_duties) || 0;
  });
