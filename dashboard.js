...
var INSIGHT_STATUS_KEY = 'crp_insight_status_v1';
function _loadInsightStatus() { try { return JSON.parse(localStorage.getItem(INSIGHT_STATUS_KEY)||'{}'); } catch(e) { return {}; } }
function _saveInsightStatus(s) { try { localStorage.setItem(INSIGHT_STATUS_KEY, JSON.stringify(s)); } catch(e) {} }
function setInsightStatus(key, status) {
  var s = _loadInsightStatus();
  if (s[key] === status) { delete s[key]; } else { s[key] = status; }
  _saveInsightStatus(s);
...
function buildInsights() {
  ...
  const insightStatus = _loadInsightStatus();
  const today = new Date();
  const flagItems = [];

  // Helper: wrap insight HTML with done/N/A buttons
  function wrapInsight(id, cssClass, iconEmoji, titleHtml, descHtml, isUrgent) {
    const ek = id.replace(/'/g,"\\'");
    const st = insightStatus[id];
    if (st) return null; // hidden if done/na
    return {
      urgent: isUrgent || false,
      html: `<div class="insight-item ${cssClass}" style="position:relative;">
        <span class="insight-icon">${iconEmoji}</span>
        <div style="flex:1;min-width:0;">
          <div class="insight-title">${titleHtml}</div>
          <div class="insight-desc">${descHtml}</div>
        </div>
        <div style="display:flex;gap:4px;flex-shrink:0;align-self:flex-start;margin-top:2px;">
          <button onclick="event.stopPropagation();setInsightStatus('${ek}','done')" style="font-size:9px;font-weight:600;padding:2px 6px;border-radius:4px;border:1px solid #d1d5db;background:#fff;cursor:pointer;color:#94a3b8;transition:all .15s;" onmouseover="this.style.borderColor='#16a34a';this.style.color='#16a34a'" onmouseout="this.style.borderColor='#d1d5db';this.style.color='#94a3b8'">Done</button>
          <button onclick="event.stopPropagation();setInsightStatus('${ek}','na')" style="font-size:9px;font-weight:600;padding:2px 6px;border-radius:4px;border:1px solid #d1d5db;background:#fff;cursor:pointer;color:#94a3b8;transition:all .15s;" onmouseover="this.style.borderColor='#94a3b8'" onmouseout="this.style.borderColor='#d1d5db'">N/A</button>
        </div>
      </div>`
    };
  }
  ...
  flags.forEach(f => {
    ...
    const visitDate = new Date(today.getFullYear(), mMap[parts[0]], parseInt(parts[1]));
    ...
    const sLnk = f.study_url
      ...
      : f.study;
    const sevLabel = f.severity === 'critical' ? ' <span style="font-size:9px;font-weight:800;padding:1px 4px;border-radius:3px;background:#dc2626;color:#fff;">CRITICAL</span>' : f.severity === 'high' ? ' <span style="font-size:9px;font-weight:800;padding:1px 4px;border-radius:3px;background:#ea580c;color:#fff;">HIGH</span>' : '';
    const id = 'risk-' + (f.patient + '-' + f.study).replace(/[^a-z0-9]/gi, '-').toLowerCase();
    const item = wrapInsight(id, urgent ? 'danger' : 'warning', urgent ? '🔴' : '⚠️',
      `${pLink} — ${f.cancels}× cancelled${sevLabel}`,
      `${sLnk} · Next visit: <strong>${f.next_visit}</strong>${urgent ? ' <span style="color:#dc2626;font-weight:700;">— CALL NOW</span>' : ''}`, urgent);
    if (item) flagItems.push(item);
  });

  // High cancel studies
  cancels.filter(c => c.count >= 15).forEach(c => {
    const sLnk2 = c.study_url
    ...
    const id = 'hicx-' + (c.name||'').replace(/[^a-z0-9]/gi,'-').toLowerCase();
    const item = wrapInsight(id, 'warning', '📉', `${sLnk2} — high cancellation volume`, `${c.count} cancelled visits in last 2 months · Review retention strategy`);
    if (item) flagItems.push(item);
  });
  ...
  const thisWeek = detail.filter(d => {
    ...
    const mMap = {Jan:0,Feb:1,Mar:2,Apr:3,May:4,Jun:5,Jul:6,Aug:7,Sep:8,Oct:9,Nov:10,Dec:11};
    const daysOut = Math.round((new Date(today.getFullYear(), mMap[parts[0]], parseInt(parts[1])) - today) / 86400000);
    return daysOut <= 7;
  });
  if (thisWeek.length > 0) {
    ...
    const wkItem = wrapInsight('thisweek-visits', 'warning', '📅',
      `${thisWeek.length} visits scheduled this week (${uniqueThisWeek.length} patients)`,
      `Confirm attendance — ${uniqueThisWeek.slice(0, 15).map(d => {
        ...
        return d.patient_url
          ...
          : label;
      }).join(', ')}${uniqueThisWeek.length > 15 ? ' +' + (uniqueThisWeek.length - 15) + ' more' : ''}`);
    if (wkItem) flagItems.push(wkItem);
  }

  // Cross-source: referrals without med records
  var refsNoMR = findReferralsWithoutMedRecords();
  if (refsNoMR.length > 0) {
    const mrItem = wrapInsight('refs-no-medrec', 'warning', '📋',
      `${refsNoMR.length} active referrals have no medical records entry`,
      `Create medical records entries for these referrals to track their intake progress`);
    if (mrItem) flagItems.push(mrItem);
  }

  // Cross-source: ready to schedule but no visit
  var readyNV = findReadyNoVisit();
  if (readyNV.length > 0) {
    const rvItem = wrapInsight('ready-no-visit', readyNV.length >= 3 ? 'danger' : 'warning', '📅',
      `${readyNV.length} patient${readyNV.length>1?'s':''} ready to schedule but no upcoming visit`,
      `${readyNV.slice(0,3).map(r => maskPHI(r.name)).join(', ')}${readyNV.length>3 ? ' +' + (readyNV.length-3) + ' more' : ''} — schedule appointments now`,
      readyNV.length >= 3);
    if (rvItem) flagItems.push(rvItem);
  }
  ...
  if (staleRefs > 0) {
    const stItem = wrapInsight('stale-referrals', 'warning', '⏰',
      `${staleRefs} referrals stale for 14+ days`,
      `Review and update stale leads in the referral pipeline to prevent drop-off`);
    if (stItem) flagItems.push(stItem);
  }
  ...
  if (statusEntries.length > 1) {
    ...
    const cxItem = wrapInsight('cx-by-stage', 'info', '📊',
      `Cancellation breakdown by enrollment stage`, breakdown);
    if (cxItem) flagItems.push(cxItem);
  }

  // Sort urgent first
  flagItems.sort((a, b) => (b.urgent ? 1 : 0) - (a.urgent ? 1 : 0));
  // Count cleared insights
  var clearedInsights = Object.keys(insightStatus).filter(k => insightStatus[k]);
  var clearedHtml = '';
  if (clearedInsights.length > 0) {
    clearedHtml = `<div style="padding:8px 0;text-align:center;">
      <button onclick="var x=document.getElementById('insight-cleared-list');x.style.display=x.style.display==='none'?'block':'none'" style="background:none;border:none;font-size:11px;color:#94a3b8;cursor:pointer;text-decoration:underline dotted;">
        ${clearedInsights.length} cleared — click to show
      </button>
      <div id="insight-cleared-list" style="display:none;margin-top:6px;text-align:left;">
        ${clearedInsights.map(k => {
          const st = insightStatus[k];
          const ek = k.replace(/'/g,"\\'");
          return `<div style="display:flex;gap:6px;align-items:center;padding:4px 0;border-bottom:1px solid #f8fafc;opacity:0.5;">
            <span style="font-size:9px;font-weight:700;padding:1px 5px;border-radius:3px;background:${st==='done'?'#f0fdf4':'#f8fafc'};color:${st==='done'?'#16a34a':'#94a3b8'}">${st==='done'?'Done':'N/A'}</span>
            <span style="font-size:11px;flex:1;text-decoration:line-through;color:#94a3b8;">${k}</span>
            <button onclick="setInsightStatus('${ek}','undo')" style="font-size:9px;border:1px solid #e2e8f0;border-radius:3px;padding:1px 6px;background:none;cursor:pointer;color:#94a3b8">Undo</button>
          </div>`;
        }).join('')}
      </div>
    </div>`;
  ...
  flagEl.innerHTML = (flagItems.length > 0 ? flagItems.map(f => f.html).join('') :
    '<div class="insight-item positive"><span class="insight-icon">✅</span><div><div class="insight-title">No critical flags</div></div></div>') + clearedHtml;
...
async function fetchLookerStudyStatus() {
  var url = CRP_CONFIG.DATA_FEEDS.LOOKER_STUDY_STATUS_CSV;
  if (!url) return;
  try {
