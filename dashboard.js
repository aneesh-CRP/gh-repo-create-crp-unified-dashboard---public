...
function buildMedRecAlerts() {
  ...
  if (refsNoMR.length > 0) {
    alerts.push({ icon: '⚠️', color: '#d97706', bg: '#fffbeb',
      ...
      action: 'Create medical records entries in ClickUp for these patients to track intake progress.'
    });
  }
  ...
  if (readyNoVisit.length > 0) {
    alerts.push({ icon: '📅', color: '#dc2626', bg: '#fef2f2',
      ...
      action: 'Schedule visits for these patients immediately — they are cleared and waiting.'
    });
  }
  ...
  if (stale.length > 0) {
    alerts.push({ icon: '⏰', color: '#f59e0b', bg: '#fffbeb',
      ...
      detail: stale.sort((a,b) => b.days_since_update - a.days_since_update).slice(0, 5).map(r => '<div style="font-size:11px;padding:2px 0;">' + maskPHI(r.name) + ' — ' + r.days_since_update + ' days — ' + escapeHTML(r.study||'') + '</div>').join(''),
      action: 'Review these records and update status. Stale records may indicate lost patients.'
    });
  }
  ...
  if (pendingApproval.length > 0) {
    alerts.push({ icon: '📋', color: '#8b5cf6', bg: '#f5f3ff',
      ...
      detail: pendingApproval.slice(0, 5).map(r => '<div style="font-size:11px;padding:2px 0;">' + maskPHI(r.name) + ' — ' + escapeHTML(r.investigator_approval) + ' — ' + escapeHTML(r.study||'') + '</div>').join(''),
      action: 'Follow up with PI to approve these patients for study enrollment.'
    });
  }

  if (alerts.length === 0) {
    el.innerHTML = '<div style="text-align:center;padding:16px;color:#059669;font-size:12px;font-weight:600;">All medical records are up to date</div>';
    if (badge) { badge.textContent = '0'; badge.className = 'badge badge-green'; }
    return;
  }

  if (badge) { badge.textContent = alerts.length + ' alert' + (alerts.length>1?'s':''); badge.className = 'badge badge-purple'; }
  ...
  el.innerHTML = alerts.map(a => `
    <div style="padding:12px 16px;border-bottom:1px solid #f1f5f9;">
      <div style="display:flex;gap:8px;align-items:flex-start;">
        ...
        <div style="flex:1;min-width:0;">
          <div style="font-size:12px;font-weight:700;color:${a.color};margin-bottom:4px;">${a.title}</div>
          <div style="margin-bottom:6px;">${a.detail}</div>
          <div style="font-size:11px;color:#64748b;padding:4px 8px;background:#f8fafc;border-radius:4px;border-left:2px solid ${a.color};">
