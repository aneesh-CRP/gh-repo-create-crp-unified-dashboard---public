...
const CRP_CONFIG = {
  ...
  SOURCE_NAME_MAP: {
    ...
    'princeton cardiometabolic':            'Dr. Savita Singh',
  ...
  .then(function(batch) {
    ...
    if (window._recruitingData.length > 0) {
      ...
      window._recruitingData.forEach(function(r) {
        ...
        if (name && r.referral_source) {
          var _srcNorm = (CRP_CONFIG.SOURCE_NAME_MAP || {})[r.referral_source.toLowerCase().trim()] || r.referral_source;
          window._crioRefSourceMap.set(name, _srcNorm);
        }
      });
