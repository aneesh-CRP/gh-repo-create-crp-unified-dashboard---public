...
function renderCoordinatorGoals() {
  ...
  grid.innerHTML = COORDS.map(name => {
    ...
    const dayBars = twoWeekDays.map((d,i) => {
      ...
      // Show actual detail count when we have patient data; fall back to snapshot count
      ...
      // Bar height uses same value as displayed number — no mismatch
