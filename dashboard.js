...
async function fetchFinanceBQ() {
  ...
  try {
    // Batch GAAP feeds (1 request instead of 4) + core finance feeds (5 requests)
    var gaapBatchP = fetch(base + '?feed=batch&feeds=gaapStudyRevenue,gaapMonthly,gaapAging,enrollmentForecast&format=json')
      .then(function(r) { return r.json(); }).catch(function() { return { results: {} }; });
    var [invRows, pmtRows, sfRows, mrevRows, stipRows, gaapBatch] = await Promise.all([
