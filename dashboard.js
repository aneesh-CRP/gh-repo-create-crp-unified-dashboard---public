...
function renderStaffEconomics() {
  ...
  if (calEl) {
    ...
    coordNames.forEach(name => {
      ...
      const taData = empMap[Object.keys(empMap).find(k => {
        const parts = name.toLowerCase().split(/\s+/);
        return parts.length >= 2 && k.toLowerCase().includes(parts[0]) && k.toLowerCase().includes(parts[parts.length-1]);
      }) || ''];
    ...
    invNames.forEach(name => {
      ...
      // Also check empMap for TA daily data
      const taData = empMap[Object.keys(empMap).find(k => {
        const parts = name.toLowerCase().split(/\s+/);
        return parts.length >= 2 && k.toLowerCase().includes(parts[0]) && k.toLowerCase().includes(parts[parts.length-1]);
      }) || ''];
