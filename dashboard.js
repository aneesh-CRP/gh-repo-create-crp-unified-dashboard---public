  ...
  if (dnqEl && unifiedRows.length > 0) {
    ...
    if (window._dnqReasons && window._dnqReasons.length > 0) {
      ...
      function _catReason(reason) {
        ...
        if (!r || r === 'changed status via recruitment api') return 'No Reason Given';
        if (r.indexOf('inclusion') !== -1 || r.indexOf('criteria') !== -1 || r.indexOf('exclusion') !== -1) return 'Inclusion/Exclusion Criteria';
        if (r.indexOf('p-tau') !== -1) return 'P-Tau Ineligible';
        if (r.indexOf('lab') !== -1) return 'Lab Results';
        if (r.indexOf('bmi') !== -1) return 'BMI';
        if (r.indexOf('age') !== -1) return 'Age';
        if (r.indexOf('medication') !== -1 || r.indexOf('drug') !== -1 || r.indexOf('cosentyx') !== -1 || r.indexOf('dupixent') !== -1 || r.indexOf('jaki') !== -1) return 'Medications';
        if (r.indexOf('withdrew') !== -1 || r.indexOf('consent') !== -1) return 'Withdrew Consent';
        if (r.indexOf('no show') !== -1 || r.indexOf('no response') !== -1 || r.indexOf('unable to reach') !== -1 || r.indexOf('cannot be reached') !== -1) return 'Unable to Reach';
        if (r.indexOf('not interested') !== -1 || r.indexOf('does not wish') !== -1 || r.indexOf('too far') !== -1 || r.indexOf('compensation') !== -1) return 'Patient Declined';
        if (r.indexOf('another study') !== -1) return 'In Another Study';
        if (r.indexOf('update') !== -1 || r.indexOf('data') !== -1) return 'Data Update';
        if (r.indexOf('stroke') !== -1 || r.indexOf('heart attack') !== -1 || r.indexOf('medical') !== -1) return 'Medical History';
        return 'Other';
      }
      ...
      window._dnqReasons.forEach(function(d) {
        ...
        if (!_byStatusReason[key]) _byStatusReason[key] = { status: d.newStatus, reason: cat, count: 0, sources: {} };
      ...
      _reasonRows.forEach(function(r) {
        ...
        var topSources = Object.entries(r.sources).sort(function(a,b){return b[1]-a[1];}).slice(0,3).map(function(e){return escapeHTML(e[0])+' ('+e[1]+')';}).join(', ');
