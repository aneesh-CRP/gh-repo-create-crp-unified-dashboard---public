...
const CRP_CONFIG = {
  ...
  SOURCE_NAME_MAP: {
    ...
    'dr gavarone':                          'Dr. Modarressi',
    'dr brian shaffer':                     'Dr. Modarressi',
    'delfa':                                'Meta (DELFA)',
    'facebook':                             'Meta (DELFA)',
        ...
        if (_fbNameCol) {
          ...
          window._recruitingData.forEach(function(r) {
            ...
            if (_fbNames.has(name)) {
              window._crioRefSourceMap.set(name, { source: 'Meta (DELFA)', category: 'Advertisement', date: r.patient_created || r.status_changed || '', inferred: true });
              _inferred++;
            }
          });
        }
      }
      _log('CRP: Recruiting data loaded — ' + window._recruitingData.length + ' records, ' + _crioSourced + ' CRIO-sourced, ' + _inferred + ' inferred, ' + window._crioRefSourceMap.size + ' total mapped');
    }
