...
function renderStudyProfitability() {
  ...
  // Build labor cost per study from studyHoursAll
  ...
  CRIO_STUDIES_DATA.forEach(function(s) {
    ...
    // Find labor cost (fuzzy match on protocol number)
    var labor = 0;
...
function buildSchedulingGapAlerts() {
  ...
  var SKIP = {'Config Study':1,'Upload test':1,'EVENT':1,'2025_COVID_FLU_RSV_DETECTION STUDY':1};
  // Studies to exclude from scheduling gap alerts (visits scheduled externally or not applicable)
