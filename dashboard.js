...
function buildHorizon() {
  ...
  // Build a unified 8-week timeline — each week shows both cancels and upcoming
  const allWeeks = [];
  cancelWeeks.forEach(w => { allWeeks.push({ week: w.week, cancels: w.count, upcoming: 0, isPast: true }); });
  upcomingWeeks.forEach(w => { allWeeks.push({ week: w.week, cancels: 0, upcoming: w.count, isPast: false }); });

  // Find today's position
  const now = new Date();
  const todayWeekLabel = new Date(now.getFullYear(), now.getMonth(), now.getDate() - (now.getDay() === 0 ? 6 : now.getDay() - 1))
    .toLocaleDateString('en-US', { month: 'short', day: 'numeric' });

  let html = '';
  allWeeks.forEach((w, i) => {
    const isThisWeek = w.week === todayWeekLabel;
    const border = isThisWeek ? 'border:2px solid #059669;' : '';
    const bg = w.isPast ? 'background:#fef2f2;' : 'background:#eff6ff;';
    const clickType = w.isPast ? 'cancel' : 'upcoming';
    const clickCount = w.isPast ? w.cancels : w.upcoming;
    html += `<div style="flex:1;min-width:90px;padding:10px 6px;border-radius:8px;text-align:center;cursor:pointer;${bg}${border}" title="${clickCount} ${w.isPast ? 'cancellations' : 'visits'} week of ${w.week}" onclick="showHorizonDetail('${clickType}','${w.week}')">
      <div style="font-size:10px;font-weight:600;color:#94a3b8;margin-bottom:4px;">${w.week}${isThisWeek ? ' ★' : ''}</div>`;
    if (w.isPast) {
      html += `<div style="font-size:20px;font-weight:700;color:#dc2626;">${w.cancels}</div>
        <div style="font-size:9px;color:#94a3b8;">cancels</div>`;
    } else {
      html += `<div style="font-size:20px;font-weight:700;color:#1843ad;">${w.upcoming}</div>
        <div style="font-size:9px;color:#94a3b8;">visits</div>`;
    }
    html += `</div>`;
  });
...
function renderRecruiterPerformance() {
  ...
  // ── Supplement with BQ recruiter outreach stats (calls/texts/emails) ──
  if (window._recruiterStats && window._recruiterStats.length > 0) {
    ...
    html += '<div style="font-size:11px;font-weight:700;color:#475569;margin-bottom:8px;">Outreach Activity (30 days — from CRIO)</div>';
    html += '<table style="width:100%;font-size:11px;border-collapse:collapse;"><thead><tr style="border-bottom:1px solid #e2e8f0;">';
    html += '<th style="text-align:left;padding:4px 6px;color:#94a3b8;font-weight:600;">Recruiter</th>';
    html += '<th style="text-align:right;padding:4px 6px;color:#94a3b8;font-weight:600;">Calls</th>';
    html += '<th style="text-align:right;padding:4px 6px;color:#94a3b8;font-weight:600;">Texts</th>';
    html += '<th style="text-align:right;padding:4px 6px;color:#94a3b8;font-weight:600;">Emails</th>';
    html += '<th style="text-align:right;padding:4px 6px;color:#94a3b8;font-weight:600;">Interested</th>';
    html += '<th style="text-align:right;padding:4px 6px;color:#94a3b8;font-weight:600;">Patients</th>';
    html += '</tr></thead><tbody>';
    window._recruiterStats.forEach(function(r) {
      html += '<tr style="border-bottom:1px solid #f8fafc;">';
      html += '<td style="padding:4px 6px;font-weight:600;">' + escapeHTML(r.recruiter || '') + '</td>';
      html += '<td style="text-align:right;padding:4px 6px;">' + (r.phone_calls || 0) + '</td>';
      html += '<td style="text-align:right;padding:4px 6px;">' + (r.texts || 0) + '</td>';
      html += '<td style="text-align:right;padding:4px 6px;">' + (r.emails || 0) + '</td>';
      html += '<td style="text-align:right;padding:4px 6px;color:#059669;font-weight:600;">' + (r.interested_responses || 0) + '</td>';
      html += '<td style="text-align:right;padding:4px 6px;">' + (r.unique_patients || 0) + '</td>';
      html += '</tr>';
    });
...
function fetchActionRequiredData() {
  ...
  fetch(base + '?feed=batch&feeds=comments,documentSummary,visitTodos,regulatory,recruiterStats,demographics&format=json')
  ...
  .then(function(batch) {
    ...
    _log('Action Required: queries=' + comments.length + ', unsignedDocs=' + unsignedTotal + ', overdueTodos=' + overdue.length + ', trainingGaps=' + missingTotal + ', recruiters=' + window._recruiterStats.length);
