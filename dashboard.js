...
function renderReferralDashboard() {
  ...
  trackerEl.innerHTML = `<table class="fin-table" style="width:100%;font-size:11px;">
    <thead><tr>
      ...
      <th style="text-align:center;color:#f97316;cursor:pointer;" ${_sortClick('sf')}>Screen Fail${_sortIcon('sf')}</th>
      <th style="text-align:center;color:#dc2626;cursor:pointer;" ${_sortClick('notEligible')}>Not Eligible${_sortIcon('notEligible')}</th>
      <th style="text-align:center;color:#94a3b8;cursor:pointer;" ${_sortClick('notInterested')}>Not Interested${_sortIcon('notInterested')}</th>
    ...
    <tr style="border-top:2px solid var(--border);background:var(--surface2);font-weight:700;">
      ...
      <td style="text-align:center;color:#f97316;">${unifiedRows.reduce((s,r)=>s+(r.sf||0),0)}</td>
      <td style="text-align:center;color:#dc2626;">${unifiedRows.reduce((s,r)=>s+(r.notEligible||0),0)}</td>
      <td style="text-align:center;color:#94a3b8;">${unifiedRows.reduce((s,r)=>s+(r.notInterested||0),0)}</td>
