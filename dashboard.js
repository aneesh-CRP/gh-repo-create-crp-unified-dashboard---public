...
// ═══ DATE PARSING (global — safe for YYYY-MM-DD) ═══
// Forces local-time parsing: 'YYYY-MM-DD' → 'YYYY-MM-DDT00:00:00' to avoid UTC off-by-one
...
if ('serviceWorker' in navigator && !(typeof google !== 'undefined' && google.script && google.script.run)) {
  ...
  if (_lastSW && _lastSW !== _swVersion) {
    navigator.serviceWorker.getRegistrations().then(function(regs) {
      ...
      localStorage.setItem('crp_sw_version', _swVersion);
      _log('CRP: SW force-updated from ' + _lastSW + ' to ' + _swVersion);
    });
  }
  localStorage.setItem('crp_sw_version', _swVersion);
  // Show version in data health strip
  var _vEl = document.getElementById('dh-version');
  if (_vEl) _vEl.textContent = 'SW ' + _swVersion;
  ...
  navigator.serviceWorker.register(swPath, { updateViaCache: 'none' }).then(function(reg) {
    ...
    reg.addEventListener('updatefound', function() {
      ...
      if (newSW) newSW.addEventListener('statechange', function() {
        if (newSW.state === 'activated') { _log('CRP: New SW activated'); }
      });
    });
  }).catch(function(err) {
...
const CRP_CONFIG = {
  ...
  CF_BASE: 'https://us-east1-crio-468120.cloudfunctions.net/crp-bq-feeds',
  USE_CLOUD_FUNCTION: true,

  // Manual enrollment targets — overrides CRIO target_enrollment when CRIO has 0
  // Key: protocol_number (substring match), Value: target
  ENROLLMENT_TARGETS: {
    '80202135SJS3001': 3,
    'I8F-MC-GPHE': 4,
    'J2A-MC-GZPO': 5,
  ...
  INVESTIGATORS: ['Taher Modarressi','Eugene Andruczyk','Lolita Vaughan','Michael Tomeo','Joseph Heether','Jason Schoenfeld','Donna Gavarone','Lawrence Leventhal','Hal Ganzman','Savita Singh','Brian Shaffer','Parth Patel'],
  // CRIO referral_source → canonical provider name (normalizes inconsistent CRIO naming)
  SOURCE_NAME_MAP: {
    // Dr. Modarressi practice
    'dr. taher modarressit':                'Dr. Modarressi',
    ...
    // Dr. Brian Shaffer / CPCM (same practice)
    'dr brian shaffer':                     'Dr. Brian Shaffer / CPCM',
    'center for primary care medicine':     'Dr. Brian Shaffer / CPCM',
    // Other providers
    'dr gavarone':                          'Dr. Gavarone',
    'dr. savita singh':                     'Dr. Savita Singh',
    'connolly dermatology':                 'Connolly Dermatology',
    'prohealth associates':                 'Prohealth Associates',
    'parkwood medical practice':            'Parkwood',
    'parkwood':                             'Parkwood',
    'aura derm':                            'Aura Derm',
    'princeton derm':                       'Princeton Dermatology',
    'dr.elliott frank':                     'Dr. Elliott Frank',
    'dr. devi patel':                       'Dr. Devi Patel',
    'dr tiffany tsang':                     'Dr. Tiffany Tsang',
    'anthony santoro, md':                  'Anthony Santoro, MD',
    'oxford circle family medicine':        'Oxford Circle Family Medicine',
    'ashton family medicine':               'Ashton Family Medicine',
    'brian stein internal medicine':         'Brian Stein Internal Medicine',
    'frankford avenue family practice':     'Frankford Avenue Family Practice',
    'burstein medical associates':          'Burstein Medical Associates',
    'woodhaven family medicine':            'Woodhaven Family Medicine',
    'hunterdon gastroenterology associates':'Hunterdon Gastroenterology',
    'stoltz & hahn medical associates':     'Stoltz & Hahn Medical',
    'tri-county rheumatology':              'Tri-County Rheumatology',
    'murray brand & associates':            'Murray Brand & Associates',
    // Digital marketing / vendors
    'delfa':                                'DELFA',
    'facebook':                             'DELFA',
    'study team':                           'Study Team',
    'study teams':                          'Study Team',
    'study max':                            'Study Max',
    ...
    'tristar':                              'Tristar',
    'iconnect':                             'iConnect',
    'x-factor smart':                       'X-Factor SMART',
    'us recruitment':                       'US Recruitment',
    // Other
    'senior expo':                          'Senior Expo',
  ...
  FETCH_TIMEOUT: 30000,         // abort fetch after 30s
  WORKER_TIMEOUT: 10000,        // abort Web Worker after 10s
  RETRY_DELAY: 5000,            // delay before retry on initial load failure
  TOAST_DURATION: 5000,         // default toast notification duration
  FRESHNESS_INTERVAL: 60000,    // update freshness badge every 60s
...
function showStudyModal(s){
  // Redirect to unified modal if available (shows all data in one place)
...
function initColl(){
  if (!UNPAID_INVOICES || !UNPAID_INVOICES.length) return;
  collData=UNPAID_INVOICES.map(inv=>{const k='crp_coll_'+inv.invoice;const s=localStorage.getItem(k);
    if(s){try{const p=JSON.parse(s);return{...inv,...p};}catch(e){}}
...
function renderFinMarketingROI() {
  ...
  camps.forEach(function(c) {
    ...
    html += '<td class="r" style="font-weight:700;color:#1843AD;">' + c.leads.toLocaleString() + '</td>';
...
var AUTH_LOCKOUT_MS = 60000; // 1 minute lockout

async function verifyPin() {
  // Rate-limit: check lockout
  if (Date.now() < _authLockUntil) {
    var secLeft = Math.ceil((_authLockUntil - Date.now()) / 1000);
    var errEl = document.getElementById('authError');
    if (errEl) { errEl.textContent = 'Too many attempts. Try again in ' + secLeft + 's'; errEl.style.display = 'block'; }
    return;
  }
  ...
  if (hash === FIN_PIN_HASH) {
    ...
    if (_authAttempts >= AUTH_MAX_ATTEMPTS) {
      _authLockUntil = Date.now() + AUTH_LOCKOUT_MS;
      if (errEl) { errEl.textContent = 'Too many attempts. Locked for 60 seconds.'; errEl.style.display = 'block'; }
    } else {
      if (errEl) { errEl.textContent = 'Incorrect PIN. ' + (AUTH_MAX_ATTEMPTS - _authAttempts) + ' attempts remaining.'; errEl.style.display = 'block'; }
    }
    document.getElementById('authPinInput').value = '';
...
function togglePHIMask() {
  ...
  try { if (typeof renderAll === 'function') renderAll(); } catch(e) { _log('renderAll: ' + e.message); }
  try { if (typeof renderStudiesTable === 'function') renderStudiesTable(); } catch(e) { _log('renderStudiesTable: ' + e.message); }
  try { if (typeof renderReferralDashboard === 'function' && _referralsLoaded) renderReferralDashboard(); } catch(e) { _log('renderReferralDashboard: ' + e.message); }
  // Mask/unmask pre-rendered static tables
  try { maskStaticPHI(); } catch(e) { _log('maskStaticPHI: ' + e.message); }
}

let finInitDone = false;
// ═══ REVENUE PER USER (Productivity Tab) ═══
var _rpuData = null;
var _rpuDays = 365;
...
async function loadRevenuePerUser(days, btn) {
  ...
  if (btn) {
    btn.closest('div').querySelectorAll('.filter-btn').forEach(function(b){b.classList.remove('active');});
    btn.classList.add('active');
  }
  var badge = document.getElementById('rpu-badge');
  if (badge) badge.textContent = 'Loading...';
  try {
    var cfBase = (CRP_CONFIG.USE_CLOUD_FUNCTION && CRP_CONFIG.CF_BASE) ? CRP_CONFIG.CF_BASE : null;
    if (!cfBase) { if (badge) badge.textContent = 'No cloud function'; return; }
    var now = new Date();
    ...
    if (badge) badge.textContent = _rpuData.length + ' visits · $' + Math.round(_rpuData.reduce(function(s,r){return s+parseFloat(r.visit_revenue||0);},0)/1000) + 'K';
    renderRevenuePerUser();
    try { renderProdCancelChart(); } catch(e2) { console.error('renderProdCancelChart:', e2); }
  } catch(e) {
    console.error('loadRevenuePerUser:', e);
    if (badge) badge.textContent = 'Failed: ' + e.message;
  }
}

function renderRevenuePerUser() {
  if (!_rpuData || _rpuData.length === 0) return;
  var el = function(id) { return document.getElementById(id); };
  var totalRev = 0, totalVisits = _rpuData.length;
  var byCoord = {}, byInv = {}, byRecruiter = {}, byMonth = {};
  var sites = new Set();

  _rpuData.forEach(function(r) {
    var rev = parseFloat(r.visit_revenue || 0);
    totalRev += rev;
    sites.add(r.site_name);
    var month = r.month_completed || '';
    var studyShort = r.study_name.indexOf(' - ') !== -1 ? r.study_name.split(' - ').pop() : r.study_name;

    // Coordinator aggregation
    var c = r.coordinator || 'Unassigned';
    if (!byCoord[c]) byCoord[c] = {visits:0,revenue:0,questions:0,studies:{},site:{}};
    byCoord[c].visits++;
    byCoord[c].revenue += rev;
    byCoord[c].questions += parseInt(r.coord_questions||0);
    byCoord[c].studies[studyShort] = (byCoord[c].studies[studyShort]||0) + rev;
    byCoord[c].site[r.site_name] = (byCoord[c].site[r.site_name]||0) + rev;

    // Investigator aggregation
    var inv = r.investigator || 'Unassigned';
    if (!byInv[inv]) byInv[inv] = {visits:0,revenue:0,questions:0,studies:{}};
    byInv[inv].visits++;
    byInv[inv].revenue += rev;
    byInv[inv].questions += parseInt(r.inv_questions||0);
    byInv[inv].studies[studyShort] = (byInv[inv].studies[studyShort]||0) + rev;

    // Recruiter aggregation (first person who scheduled the visit)
    var rec = r.recruiter || 'Unassigned';
    if (!byRecruiter[rec]) byRecruiter[rec] = {visits:0,revenue:0,questions:0,studies:{},site:{}};
    byRecruiter[rec].visits++;
    byRecruiter[rec].revenue += rev;
    byRecruiter[rec].studies[studyShort] = (byRecruiter[rec].studies[studyShort]||0) + rev;
    byRecruiter[rec].site[r.site_name] = (byRecruiter[rec].site[r.site_name]||0) + rev;

    // Monthly trend
    if (month) {
      if (!byMonth[month]) byMonth[month] = {};
      byMonth[month][c] = (byMonth[month][c]||0) + rev;
    }
  });
  ...
  if (_rpuCostData) {
    var totalCostAll = Object.values(_rpuCostData).reduce(function(s,d){return s+d.cost;},0);
    var totalHrsAll = Object.values(_rpuCostData).reduce(function(s,d){return s+d.hours;},0);
    var marginAll = totalRev - totalCostAll;
    _kpi('rpu-kpi-cost', '$' + Math.round(totalCostAll/1000).toLocaleString() + 'K');
    var costSub = el('rpu-kpi-cost-sub'); if (costSub) costSub.textContent = Math.round(totalHrsAll).toLocaleString() + 'h tracked';
    _kpi('rpu-kpi-margin', '$' + Math.round(marginAll/1000).toLocaleString() + 'K');
    var marginSub = el('rpu-kpi-margin-sub'); if (marginSub) marginSub.textContent = (totalRev > 0 ? Math.round(marginAll/totalRev*100) : 0) + '% margin';
    var marginEl = el('rpu-kpi-margin'); if (marginEl) marginEl.style.color = marginAll >= 0 ? '#059669' : '#dc2626';
  }

  // Helper: build user table
  function buildUserTable(data, labelBadgeId, label, opts) {
    opts = opts || {};
    var showCost = opts.showCost !== false && _rpuCostData && Object.keys(_rpuCostData).length > 0;
    // Filter: only show specific role names if provided
    var allowList = opts.allowList || null;
    var blockList = opts.blockList || null;

    var rows = Object.entries(data).filter(function(e){
      if (e[0]==='Unassigned') return false;
      if (e[1].visits === 0 && e[1].revenue === 0) return false; // remove stale/empty
      if (allowList && !allowList.some(function(a){return e[0].toLowerCase().includes(a.toLowerCase().split(' ').pop());})) return false;
      if (blockList && blockList.some(function(b){return e[0].toLowerCase().includes(b.toLowerCase().split(' ').pop());})) return false;
      return true;
    }).sort(function(a,b){return b[1].revenue-a[1].revenue;});
    var maxRev = rows.length > 0 ? rows[0][1].revenue : 1;
    var badge = el(labelBadgeId);
    if (badge) badge.textContent = rows.length + ' ' + label + 's · $' + Math.round(rows.reduce(function(s,r){return s+r[1].revenue;},0)/1000) + 'K';

    var h = '<table class="fin-table" style="width:100%;font-size:11px;"><thead><tr>';
    h += '<th style="text-align:left;padding:8px 12px;">' + escapeHTML(label) + '</th>';
    h += '<th style="text-align:center;">Visits</th>';
    h += '<th style="text-align:center;">Revenue</th>';
    if (showCost) {
      h += '<th style="text-align:center;">$/hr</th>';
      h += '<th style="text-align:center;">Hours</th>';
      h += '<th style="text-align:center;">Cost</th>';
      h += '<th style="text-align:center;">Margin</th>';
    }
    h += '<th style="text-align:center;">Avg/Visit</th>';
    h += '<th style="text-align:left;">Top Study</th>';
    h += '<th style="text-align:center;min-width:100px;">Revenue</th>';
    h += '</tr></thead><tbody>';
    rows.forEach(function(e) {
      var name = e[0], d = e[1];
      var avgV = d.visits > 0 ? Math.round(d.revenue / d.visits) : 0;
      var barW = maxRev > 0 ? Math.round(d.revenue / maxRev * 100) : 0;
      var topStudy = Object.entries(d.studies).sort(function(a,b){return b[1]-a[1];})[0];
      var siteInfo = d.site ? Object.entries(d.site).map(function(s){return s[0].replace(', PA','').replace(', NJ','')+'($'+Math.round(s[1]/1000)+'K)';}).join(' ') : '';
      var cName = typeof canonicalStaffName === 'function' ? canonicalStaffName(name) : name;
      var costInfo = _rpuCostData ? (_rpuCostData[cName] || _rpuCostData[name] || null) : null;
      var pCost = costInfo ? costInfo.cost : 0;
      var pHrs = costInfo ? costInfo.hours : 0;
      var pRate = costInfo ? costInfo.rate : 0;
      var margin = d.revenue - pCost;
      var mColor = margin >= 0 ? '#059669' : '#dc2626';

      h += '<tr class="clickable" style="border-bottom:1px solid #f1f5f9;cursor:pointer;" onclick="showRPUUserModal(\''+jsAttr(name)+'\',\''+jsAttr(label)+'\')">';
      h += '<td style="padding:7px 12px;"><span style="font-weight:600;">'+escapeHTML(name)+'</span>';
      if (siteInfo) h += '<div style="font-size:9px;color:#94a3b8;">'+escapeHTML(siteInfo)+'</div>';
      h += '</td>';
      h += '<td style="text-align:center;font-weight:700;">'+d.visits+'</td>';
      h += '<td style="text-align:center;font-weight:700;color:#059669;">$'+Math.round(d.revenue).toLocaleString()+'</td>';
      if (showCost) {
        h += '<td style="text-align:center;color:#64748b;font-size:10px;">'+(pRate>0?'$'+pRate.toFixed(0):'—')+'</td>';
        h += '<td style="text-align:center;color:#64748b;font-size:10px;">'+(pHrs>0?Math.round(pHrs).toLocaleString()+'h':'—')+'</td>';
        h += '<td style="text-align:center;color:#dc2626;font-weight:'+(pCost>0?'600':'400')+';">'+(pCost>0?'$'+Math.round(pCost).toLocaleString():'—')+'</td>';
        h += '<td style="text-align:center;font-weight:700;color:'+mColor+';">'+(pCost>0?'$'+Math.round(margin).toLocaleString():'—')+'</td>';
      }
      h += '<td style="text-align:center;">$'+avgV.toLocaleString()+'</td>';
    ...
    // Total
    var totRev = rows.reduce(function(s,e){return s+e[1].revenue;},0);
    var totVis = rows.reduce(function(s,e){return s+e[1].visits;},0);
    var totCost = 0, totHrs = 0;
    if (showCost) { rows.forEach(function(e) { var cn = typeof canonicalStaffName === 'function' ? canonicalStaffName(e[0]) : e[0]; var ci = _rpuCostData ? (_rpuCostData[cn] || _rpuCostData[e[0]] || null) : null; if (ci) { totCost += ci.cost; totHrs += ci.hours; } }); }
    h += '<tr style="border-top:2px solid var(--border);background:var(--surface2);font-weight:700;">';
    h += '<td style="padding:8px 12px;">TOTAL</td>';
    h += '<td style="text-align:center;">'+totVis+'</td>';
    h += '<td style="text-align:center;color:#059669;">$'+Math.round(totRev).toLocaleString()+'</td>';
    if (showCost) {
      h += '<td></td><td style="text-align:center;">'+Math.round(totHrs).toLocaleString()+'h</td>';
      h += '<td style="text-align:center;color:#dc2626;">$'+Math.round(totCost).toLocaleString()+'</td>';
      h += '<td style="text-align:center;color:'+(totRev-totCost>=0?'#059669':'#dc2626')+';">$'+Math.round(totRev-totCost).toLocaleString()+'</td>';
    }
    h += '<td style="text-align:center;">$'+(totVis>0?Math.round(totRev/totVis):0).toLocaleString()+'</td>';
  ...
  var coordEl = el('rpu-coord-table');
  // Coordinators = SCHEDULE_COORDINATORS only, with cost
  var schedCoords = CRP_CONFIG.SCHEDULE_COORDINATORS || [];
  if (coordEl) coordEl.innerHTML = buildUserTable(byCoord, 'rpu-coord-badge', 'Coordinator', { showCost: true, allowList: schedCoords });

  // Investigators with cost
  var invEl = el('rpu-inv-table');
  var invList = CRP_CONFIG.INVESTIGATORS || [];
  if (invEl) invEl.innerHTML = buildUserTable(byInv, 'rpu-inv-badge', 'Investigator', { showCost: true, allowList: invList });

  // Recruiters only — exclude coordinators and investigators
  var recEl = el('rpu-rec-table');
  var allBlockList = schedCoords.concat(invList);
  if (recEl) recEl.innerHTML = buildUserTable(byRecruiter, 'rpu-rec-badge', 'Recruiter', { showCost: false, blockList: allBlockList });

  // Make KPI sections clickable
  var kpiSections = document.querySelectorAll('#view-fin-productivity .tb-section');
  if (kpiSections[0]) { kpiSections[0].style.cursor = 'pointer'; kpiSections[0].onclick = function(){ showRPUBreakdownModal('revenue'); }; }
  if (kpiSections[1]) { kpiSections[1].style.cursor = 'pointer'; kpiSections[1].onclick = function(){ showRPUBreakdownModal('visits'); }; }
  ...
  // Monthly trend heatmap — unified coordinator + investigator grid
  var trendEl = el('rpu-monthly-trend');
  var trendBadge = el('rpu-trend-badge');
  if (trendEl) {
    // Build ONE unified monthly data structure: byMonthAll[month][person] = revenue
    // Each visit credits one coordinator AND one investigator (no dedup between roles)
    var byMonthAll = {};
    var userRoles = {}; // person → 'Coord' or 'PI'
    _rpuData.forEach(function(r) {
      var month = r.month_completed || '';
      var rev = parseFloat(r.visit_revenue || 0);
      if (!month || rev <= 0) return;
      if (!byMonthAll[month]) byMonthAll[month] = {};
      // Credit coordinator
      var coord = r.coordinator || '';
      if (coord && coord !== 'Unassigned') {
        byMonthAll[month][coord] = (byMonthAll[month][coord]||0) + rev;
        if (!userRoles[coord]) userRoles[coord] = 'Coord';
      }
      // Credit investigator
      var inv = r.investigator || '';
      if (inv && inv !== 'Unassigned') {
        byMonthAll[month][inv] = (byMonthAll[month][inv]||0) + rev;
        if (!userRoles[inv]) userRoles[inv] = 'PI';
      }
    });

    var months = Object.keys(byMonthAll).sort().slice(-12);

    // Build user list sorted by total revenue, filtered to config lists
    var userTotals = {};
    months.forEach(function(m) {
      Object.entries(byMonthAll[m] || {}).forEach(function(e) {
        userTotals[e[0]] = (userTotals[e[0]]||0) + e[1];
      });
    });

    var coordUsers = Object.keys(userTotals).filter(function(name) {
      return schedCoords.some(function(c){return name.toLowerCase().includes(c.toLowerCase().split(' ').pop());});
    }).sort(function(a,b){return (userTotals[b]||0)-(userTotals[a]||0);}).map(function(n){return {name:n,role:'Coord'};});

    var invUsersList = Object.keys(userTotals).filter(function(name) {
    ...
    th += '<th style="text-align:left;padding:6px 8px;min-width:80px;">Name</th><th style="text-align:left;padding:6px 4px;font-size:9px;min-width:40px;">Role</th>';
    months.forEach(function(m){th+='<th style="text-align:center;padding:4px 6px;min-width:50px;">'+m.substring(5)+'/'+m.substring(2,4)+'</th>';});
    th += '<th style="text-align:center;padding:4px 8px;font-weight:700;min-width:60px;">Total</th></tr></thead><tbody>';
    ...
    allUsers.forEach(function(u, i) {
      var total = userTotals[u.name] || 0;
      if (total <= 0) return;
      var maxM = Math.max.apply(null, months.map(function(m){return (byMonthAll[m]&&byMonthAll[m][u.name])||0;}))||1;
      ...
      var roleField = u.role === 'Coord' ? 'coordinator' : 'investigator';
      ...
      months.forEach(function(m) {
        var v = (byMonthAll[m]&&byMonthAll[m][u.name])||0;
        var bg = v > 0 ? 'background:'+colors[i%colors.length]+Math.round(v/maxM*40+10).toString(16)+';' : '';
        th += '<td style="text-align:center;padding:4px 6px;font-weight:'+(v>0?'600':'400')+';color:'+(v>0?'#1e293b':'#cbd5e1')+';'+bg+(v>0?'cursor:pointer;':'')+'"'+(v>0?' onclick="showRPUMonthModal(\''+jsAttr(u.name)+'\',\''+m+'\',\''+roleField+'\')"':'')+'>'+(v > 0 ? '$'+Math.round(v/1000)+'K' : '—')+'</td>';
      });
    ...
    // Total row — sum unique visit revenue per month (not double-counted)
    ...
    var grandTotal = 0;
    months.forEach(function(m) {
      // Use coordinator totals only to avoid double-counting
      var mTotal = 0;
      coordUsers.forEach(function(u) { mTotal += (byMonthAll[m]&&byMonthAll[m][u.name])||0; });
      grandTotal += mTotal;
...
function renderProdCancelChart() {
  var canvas = document.getElementById('prodCancelChart');
  var tableEl = document.getElementById('prod-cancel-table');
  var badge = document.getElementById('prod-cancel-badge');
  if (!canvas || !DATA.allCancels || DATA.allCancels.length === 0) return;

  var COORDS = CRP_CONFIG.SCHEDULE_COORDINATORS || [];
  ...
  COORDS.forEach(function(name) {
    ...
    coordStats.push({ name: name, total: cx.length,
      noShow: cx.filter(function(c){return c.type==='No Show';}).length,
      site: cx.filter(function(c){return c.type==='Site Cancelled';}).length,
      patient: cx.filter(function(c){return c.type==='Patient Cancelled';}).length
    });
  });
  ...
  if (badge) badge.textContent = totalAll + ' cancellations';

  mkChart('prodCancelChart', {
    type: 'bar',
    data: {
      labels: sorted.map(function(r){return r.name.split(' ')[0];}),
      datasets: [
        { label: 'No Show', data: sorted.map(function(r){return r.noShow;}), backgroundColor: '#dc2626', borderRadius: 3 },
        { label: 'Site', data: sorted.map(function(r){return r.site;}), backgroundColor: '#1843ad', borderRadius: 3 },
        { label: 'Patient', data: sorted.map(function(r){return r.patient;}), backgroundColor: '#FF9933', borderRadius: 3 }
      ]
    },
    options: {
      responsive: true, maintainAspectRatio: false,
      plugins: { legend: { position: 'bottom', labels: { boxWidth: 10, font: { size: 10, family: 'IBM Plex Sans' } } } },
      scales: { x: { stacked: true, ticks: { font: { size: 10 } } }, y: { stacked: true, beginAtZero: true, ticks: { font: { size: 10 }, stepSize: 1 } } }
    }
  });

  if (tableEl) {
    var h = '<table class="fin-table" style="width:100%;font-size:11px;"><thead><tr>';
    h += '<th style="text-align:left;padding:8px 12px;">Coordinator</th><th style="text-align:center;">Total</th>';
    h += '<th style="text-align:center;color:#dc2626;">No Show</th><th style="text-align:center;color:#1843ad;">Site</th>';
    h += '<th style="text-align:center;color:#FF9933;">Patient</th><th style="text-align:center;min-width:100px;">Breakdown</th>';
    h += '</tr></thead><tbody>';
    sorted.forEach(function(r) {
      ...
      h += '<td style="text-align:center;color:#dc2626;">'+(r.noShow||'—')+'</td>';
      h += '<td style="text-align:center;color:#1843ad;">'+(r.site||'—')+'</td>';
      h += '<td style="text-align:center;color:#FF9933;">'+(r.patient||'—')+'</td>';
      h += '<td style="padding:4px 8px;"><div style="display:flex;gap:1px;height:14px;border-radius:3px;overflow:hidden;">';
      if (nsPct>0) h+='<div style="width:'+nsPct+'%;background:#dc2626;"></div>';
      if (scPct>0) h+='<div style="width:'+scPct+'%;background:#1843ad;"></div>';
      if (pcPct>0) h+='<div style="width:'+pcPct+'%;background:#FF9933;"></div>';
      var rem=100-nsPct-scPct-pcPct; if(rem>0) h+='<div style="width:'+rem+'%;background:#e2e8f0;"></div>';
      h += '</div></td></tr>';
    });
    h += '</tbody></table>';
    tableEl.innerHTML = h;
...
// ═══ RPU CLICK-THROUGH MODALS ═══
function showRPUUserModal(userName, role) {
  if (!_rpuData) return;
  var field = role === 'Coordinator' ? 'coordinator' : role === 'Recruiter' ? 'recruiter' : 'investigator';
  var visits = _rpuData.filter(function(r) { return r[field] === userName; });
  var totalRev = visits.reduce(function(s,r){ return s + parseFloat(r.visit_revenue||0); }, 0);

  // Group by study
  var byStudy = {};
  visits.forEach(function(r) {
    var sn = r.study_name || 'Unknown';
    if (!byStudy[sn]) byStudy[sn] = { visits: 0, revenue: 0, actual: 0, contracted: 0, noRate: 0 };
    byStudy[sn].visits++;
    byStudy[sn].revenue += parseFloat(r.visit_revenue||0);
    if (r.revenue_type === 'Actual') byStudy[sn].actual++;
    else if (r.revenue_type === 'Contracted') byStudy[sn].contracted++;
    else byStudy[sn].noRate++;
  });

  var h = '<div style="margin-bottom:12px;font-size:12px;color:#64748b;">' + visits.length + ' visits · $' + Math.round(totalRev).toLocaleString() + ' total revenue · ' + _rpuDays + '-day period</div>';

  // Study summary table
  h += '<table class="detail-table"><thead><tr><th>Study</th><th class="r">Visits</th><th class="r">Revenue</th><th class="r">Actual</th><th class="r">Contracted</th><th class="r">No Rate</th></tr></thead><tbody>';
  Object.entries(byStudy).sort(function(a,b){return b[1].revenue-a[1].revenue;}).forEach(function(e) {
    var s = e[0], d = e[1];
    h += '<tr><td>'+slink(s)+'</td><td class="r">'+d.visits+'</td><td class="r">'+fmt(d.revenue)+'</td><td class="r">'+d.actual+'</td><td class="r">'+d.contracted+'</td><td class="r">'+(d.noRate||'')+'</td></tr>';
  });
  h += '<tr class="total-row"><td>TOTAL</td><td class="r">'+visits.length+'</td><td class="r">'+fmt(totalRev)+'</td><td class="r">'+visits.filter(function(r){return r.revenue_type==='Actual';}).length+'</td><td class="r">'+visits.filter(function(r){return r.revenue_type==='Contracted';}).length+'</td><td class="r">'+visits.filter(function(r){return r.revenue_type==='No Rate';}).length+'</td></tr>';
  h += '</tbody></table>';

  // Visit detail table
  h += '<div style="margin-top:16px;font-size:12px;font-weight:700;color:#1e293b;">Visit Detail</div>';
  ...
  openModal(escapeHTML(userName) + ' — ' + role + ' Revenue', visits.length + ' visits · $' + Math.round(totalRev).toLocaleString(), h);
}

function showRPUBreakdownModal(mode) {
  if (!_rpuData) return;
  if (mode === 'sites') {
    var bySite = {};
    _rpuData.forEach(function(r) {
      var s = r.site_name || 'Unknown';
      if (!bySite[s]) bySite[s] = { visits: 0, revenue: 0 };
      bySite[s].visits++;
      bySite[s].revenue += parseFloat(r.visit_revenue||0);
    });
    var h = '<table class="detail-table"><thead><tr><th>Site</th><th class="r">Visits</th><th class="r">Revenue</th><th class="r">Avg/Visit</th></tr></thead><tbody>';
    var totRev = 0, totVis = 0;
    Object.entries(bySite).sort(function(a,b){return b[1].revenue-a[1].revenue;}).forEach(function(e) {
      var d = e[1]; totRev += d.revenue; totVis += d.visits;
      h += '<tr><td>'+escapeHTML(e[0])+'</td><td class="r">'+d.visits+'</td><td class="r">'+fmt(d.revenue)+'</td><td class="r">'+fmt(d.visits>0?d.revenue/d.visits:0)+'</td></tr>';
    });
    h += '<tr class="total-row"><td>TOTAL</td><td class="r">'+totVis+'</td><td class="r">'+fmt(totRev)+'</td><td class="r">'+fmt(totVis>0?totRev/totVis:0)+'</td></tr></tbody></table>';
    openModal('Revenue by Site', Object.keys(bySite).length + ' sites · $' + Math.round(totRev).toLocaleString(), h);
    return;
  }
  // Revenue / visits breakdown
  var byType = { Actual: { visits: 0, revenue: 0 }, Contracted: { visits: 0, revenue: 0 }, 'No Rate': { visits: 0, revenue: 0 } };
  _rpuData.forEach(function(r) {
    var t = r.revenue_type || 'No Rate';
    if (!byType[t]) byType[t] = { visits: 0, revenue: 0 };
    byType[t].visits++;
    byType[t].revenue += parseFloat(r.visit_revenue||0);
  });
  var totRev = _rpuData.reduce(function(s,r){return s+parseFloat(r.visit_revenue||0);},0);
  var h = '<table class="detail-table"><thead><tr><th>Revenue Type</th><th class="r">Visits</th><th class="r">Revenue</th><th class="r">% of Total</th><th class="r">Avg/Visit</th></tr></thead><tbody>';
  Object.entries(byType).forEach(function(e) {
    var d = e[1]; if (d.visits === 0) return;
    var pct = totRev > 0 ? Math.round(d.revenue/totRev*100) : 0;
    h += '<tr><td>'+escapeHTML(e[0])+'</td><td class="r">'+d.visits+'</td><td class="r">'+fmt(d.revenue)+'</td><td class="r">'+pct+'%</td><td class="r">'+fmt(d.visits>0?d.revenue/d.visits:0)+'</td></tr>';
  });
  h += '<tr class="total-row"><td>TOTAL</td><td class="r">'+_rpuData.length+'</td><td class="r">'+fmt(totRev)+'</td><td class="r">100%</td><td class="r">'+fmt(_rpuData.length>0?totRev/_rpuData.length:0)+'</td></tr></tbody></table>';

  // By study
  var byStudy = {};
  _rpuData.forEach(function(r) {
    var s = r.study_name || 'Unknown';
    if (!byStudy[s]) byStudy[s] = { visits: 0, revenue: 0 };
    byStudy[s].visits++;
    byStudy[s].revenue += parseFloat(r.visit_revenue||0);
  });
  h += '<div style="margin-top:16px;font-size:12px;font-weight:700;color:#1e293b;">By Study</div>';
  h += '<table class="detail-table" style="margin-top:4px;"><thead><tr><th>Study</th><th class="r">Visits</th><th class="r">Revenue</th><th class="r">Avg/Visit</th></tr></thead><tbody>';
  Object.entries(byStudy).sort(function(a,b){return b[1].revenue-a[1].revenue;}).forEach(function(e) {
    var d = e[1];
    h += '<tr><td>'+slink(e[0])+'</td><td class="r">'+d.visits+'</td><td class="r">'+fmt(d.revenue)+'</td><td class="r">'+fmt(d.visits>0?d.revenue/d.visits:0)+'</td></tr>';
  });
  h += '</tbody></table>';
...
function initFinanceDashboard(forceRefresh) {
  ...
  try { drawRevChart(); } catch(e) { _log('drawRevChart: ' + e.message); }
...
function updateFinanceOverviewKPIs() {
  ...
  // Show GAAP upfront/holdback breakdown if available
  var gaapSrc = window._gaapStudyData || [];
  if (gaapSrc.length > 0) {
    var gaapUpfront = gaapSrc.reduce(function(s,r){return s+(r.total_upfront||0);},0);
    var gaapHoldback = gaapSrc.reduce(function(s,r){return s+(r.total_holdback||0);},0);
    var gaapInvoiced = gaapSrc.reduce(function(s,r){return s+(r.invoiced||0);},0);
    var gaapUninvoiced = gaapSrc.reduce(function(s,r){return s+(r.uninvoiced||0);},0);
    _s('fin-kpi-rev12m-sub', 'GAAP \u00b7 Upfront $' + Math.round(gaapUpfront/1000) + 'K \u00b7 Holdback $' + Math.round(gaapHoldback/1000) + 'K');
  } else {
  ...
  if (gaapSrc.length > 0) {
    var gaapUninv = gaapSrc.reduce(function(s,r){return s+(r.uninvoiced||0);},0);
    _s('fin-kpi-uninvoiced', '$' + Math.round(gaapUninv).toLocaleString());
    _s('fin-kpi-uninvoiced-sub', 'GAAP \u00b7 ' + gaapSrc.filter(function(r){return r.uninvoiced>0;}).length + ' studies');
  } else {
    _s('fin-kpi-uninvoiced-sub', UNINVOICED ? UNINVOICED.length + ' studies' : '');
  }

  // Data source indicator
  var finSrcEl = document.getElementById('fin-data-source');
  if (finSrcEl) {
    var src = gaapSrc.length > 0 ? 'GAAP Live' : (window._financeLastFetch ? 'BQ' : 'Defaults');
    finSrcEl.textContent = src + (window._financeLastFetch ? ' \u00b7 ' + window._financeLastFetch : '');
  }
}
...
function renderInsights() {
  if (!FIN_MERGED_STUDIES || !FIN_MERGED_STUDIES.length || !MONTHLY_PAYMENTS || !AGING_INV) return;
  ...
  var _ct = typeof window._adjCancel !== 'undefined' ? window._adjCancel : (DATA.cancelTotal||0);
  const cancelRate = (DATA && _ct && DATA.upcomingTotal)
    ? Math.round(_ct / (_ct + DATA.upcomingTotal) * 100) : null;
  ...
  if (nextEl) {
    ...
    if (typeof DATA !== 'undefined' && DATA.riskFlags && DATA.riskFlags.length) {
      ...
      const urgent = DATA.riskFlags.filter(r => {
        const p=r.next_visit.split(' ');
        return Math.round((new Date(_today.getFullYear(),_mm[p[0]],parseInt(p[1]))-_today)/86400000) <= 14;
      });
      if (urgent.length) {
        actions.push({
          priority: 1,
...
var RISK_CARD_STATUS_KEY = 'crp_risk_card_status_v1';
function _loadRiskCardStatus() { try { return JSON.parse(localStorage.getItem(RISK_CARD_STATUS_KEY)||'{}'); } catch(e) { return {}; } }
function _saveRiskCardStatus(s) { try { localStorage.setItem(RISK_CARD_STATUS_KEY, JSON.stringify(s)); } catch(e) {} }
function setRiskCardStatus(key, status, btn) {
  var s = _loadRiskCardStatus();
  if (s[key] === status) { delete s[key]; } else { s[key] = status; }
  _saveRiskCardStatus(s);
  safe(buildRiskFlagCards, 'buildRiskFlagCards');
}
...
function buildRiskFlagCards() {
  ...
  // Separate critical/high severity counts for KPI
  ...
  const doneFlags = flags.filter(f => {
    const k = (f.patient+'|'+f.study).toLowerCase().replace(/\s+/g,'_');
    return riskStatus[k] === 'done';
  });
  const naFlags = flags.filter(f => {
    const k = (f.patient+'|'+f.study).toLowerCase().replace(/\s+/g,'_');
    return riskStatus[k] === 'na';
  });
...
function buildScheduleTable() {
  ...
  // Pre-screening calls and FibroScans are tracked separately via the
  // Calls/FibroScan KPI in Cancellation Analysis — not in the schedule table
  if (!visits.length) {
  ...
  // Count total cancellations + no-shows per patient for repeat-cancel warnings
...
function injectRideshareButtons() {
  ...
  rows.forEach(function(row) {
    ...
    // Check CRIO interactions for this patient
    var cells = row.querySelectorAll('td');
    ...
    var isRequested = _rideshareData[key] === true || !!crioRide;
    var tooltip = crioRide ? crioRide.note || '' : '';

    var btn = document.createElement('button');
    var fromCrio = !!crioRide && !_rideshareData[key]; // auto-detected from CRIO
    btn.style.cssText = 'font-size:9px;font-weight:700;padding:3px 6px;border-radius:4px;cursor:pointer;white-space:nowrap;min-width:60px;border:1.5px solid ' + (isRequested ? '#059669' : '#cbd5e1') + ';background:' + (isRequested ? (fromCrio ? '#dbeafe' : '#ecfdf5') : '#fff') + ';color:' + (isRequested ? (fromCrio ? '#1843AD' : '#059669') : '#94a3b8') + ';';
    btn.textContent = isRequested ? (fromCrio ? '🚗 CRIO' : '✓ Requested') : '—';
    ...
    btn.onclick = function(e) {
      ...
      _saveRideshare();
      var isOn = nowReq || !!crioRide;
      btn.textContent = isOn ? '✓ Requested' : '—';
      btn.style.borderColor = isOn ? '#059669' : '#cbd5e1';
      btn.style.background = isOn ? '#ecfdf5' : '#fff';
      btn.style.color = isOn ? '#059669' : '#94a3b8';
    };
    td.innerHTML = '';
    td.appendChild(btn);
...
function buildHorizon() {
  ...
  // Build unified 8-week timeline using same tb-section style as KPI strips
  const allWeeks = [];
...
function showHorizonDetail(type, weekLabel) {
  ...
  if (type === 'cancel') {
    ...
    rows = (DATA.allCancels || []).filter(r => {
      ...
      // Year-wrap: if date is >3 months in the future, it likely belongs to last year
...
function renderCoordinatorGoals() {
  ...
  grid.innerHTML = COORDS.map(name => {
    ...
    const dayBars = twoWeekDays.map((d,i) => {
      ...
      // Show actual detail count when we have patient data; fall back to snapshot count for bar height
      const visits = detailCount > 0 ? detailCount : snapCount;
      const barVisits = Math.max(visits, snapCount); // bar uses higher for visual
      ...
      const barH = Math.min(100, Math.round((barVisits / (weekMax * 1.2)) * 100));
  ...
  // Monthly summary table — snapshot of actual counts, with Visits/Duration toggle
  if (!monthBody) return;
...
function renderCoordCancelTypeChart() {
  ...
  // Sort by total cancels descending for the table
  var sorted = coordStats.slice().sort(function(a, b) { return b.total - a.total; });
  var totalAll = sorted.reduce(function(s, r) { return s + r.total; }, 0);
  ...
  // Chart — use all coordinators sorted by total
...
function renderInvCapacity() {
  ...
  // Filter to next 30 days for workload view
  var today = new Date(); today.setHours(0,0,0,0);
  ...
  var invStats = INVS.map(function(name) {
    ...
    // Utilization: days with visits / available days in 4-week window
...
// ═══════════════════════════════════════════════════════════════
// FEATURE: Vendor Performance Comparison
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// FEATURE: Contact Sequence Effectiveness
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// FEATURE: Referral-to-Enrollment Conversion by Source
// ═══════════════════════════════════════════════════════════════
function renderRefConversion() {
  ...
  // Cross-ref with med records if available
  var mrBySource = {};
...
// ═══════════════════════════════════════════════════════════════
// FEATURE: Medical Records Portal Filter
// ═══════════════════════════════════════════════════════════════
function renderMedRecPortalFilter() {
  ...
  // Insert filter buttons at top of container (after existing content)
...
function filterMedRecByPortal(portal) {
  ...
  // Show/hide med record rows based on portal
  // Since med rec content is rebuilt dynamically, we store the filter and re-render
...
function fetchActionRequiredData() {
  ...
  fetch(base + '?feed=batch&feeds=comments,documentSummary,visitTodos,regulatory,recruiterStats,demographics,eregPending,prescrVisits,recruiting,subjectAudit,unscheduledVisits,subjects,rideRequests,visitConfirmations&format=json')
  .then(function(r) { return r.json(); })
  .then(function(batch) {
    ...
    // CRIO recruiting data — referral source per patient
    window._recruitingData = (results.recruiting || {}).data || [];
    if (window._recruitingData.length > 0) {
      var _srcMap = CRP_CONFIG.SOURCE_NAME_MAP || {};
      // Build referral_source lookup: patient name (lower) → { source, category, patient_created, inferred }
      ...
      window._recruitingData.forEach(function(r) {
        ...
        if (rawSrc) {
          // Tier 1: CRIO has a source — normalize and use it
      ...
      // Tier 2: Infer from ClickUp tracker match (patient in provider list)
      if (REFERRAL_DATA && REFERRAL_DATA.length > 0) {
        window._recruitingData.forEach(function(r) {
          var name = (r.patient_name||'').toLowerCase().trim().replace(/\s+/g, ' ');
          if (!name || window._crioRefSourceMap.has(name)) return;
          // Find in ClickUp referral data by name match
      ...
      // Tier 3: Infer from FB CRM data (patient in Facebook lead sheet = Meta/DELFA)
    ...
    if (window._subjectAudit.length > 0) {
      // Build subject_name → referral source lookup for cross-referencing
      window._dnqReasons = [];
      ...
      // Build latest status per patient+study (truth from audit trail)
      // Used to filter out recovered patients from cancel/SF/no-show counts
    ...
    // Ride requests — auto-populate rideshare column
    ...
    // Visit confirmations — auto-populate tracking column
...
function showActionModal(type) {
  ...
  if (type === 'queries') {
    ...
    body = '<table class="detail-table"><thead><tr><th>Study</th><th>Subject</th><th>Document</th><th>Category</th><th>Assigned To</th><th>Assigned Date</th></tr></thead><tbody>' +
      rows.slice(0, 100).map(function(r) {
        ...
        var pName = escapeHTML(maskPHI(r.subject_name||''));
        var pHtml = pUrl ? '<a href="' + escapeHTML(pUrl) + '" target="_blank" style="color:#1843ad;text-decoration:none">' + pName + '</a>' : pName;
        return '<tr><td style="font-size:11px">' + _crioLink((r.study_name||'').split(' - ').pop(), studyUrl) + '</td>' +
          '<td style="font-size:11px">' + pHtml + '</td>' +
          ...
          '<td style="font-size:11px">' + escapeHTML(r.assigned_date||'—') + '</td></tr>';
      }).join('') + '</tbody></table>';
  }
...
function buildInsights() {
  ...
  // ── Flags: summary counts that link to detail sections (no duplicate detail) ──
  const flagEl = document.getElementById('perf-insight-flags');
  if (!flagEl) return;

  const flagItems = [];

  // At-risk patients — summary linking to Schedule tab risk cards
  ...
  // High cancel studies — unique to Overview (study-level, not patient-level)
  ...
  // Action plan summary — link to Schedule tab
  ...
  // Med records gaps — summary linking to Schedule tab
  ...
  // Stale referrals — unique to Overview (referral pipeline health)
  ...
  // Cancel breakdown by status — unique analytic (not duplicated elsewhere)
...
async function fetchFinanceLive() {
  ...
  try {
    ...
    // Re-render all finance views with fresh data
...
function processLiveData(allRows, legacyCancels, auditLog) {
  ...
  // ════════════════════════════════════════════════════════════
  // PHASE 1: Split report into upcoming vs cancels, dedup
  // ════════════════════════════════════════════════════════════
  ...
  // Build COORD_MAP from config (coordinators + investigators) for case normalization
  const COORD_MAP = {};
  ...
  // ── Derive Site Name from Study Key (authoritative) ──
  // The upcoming visits CSV may have no Site Name or may show "Philadelphia, PA" for all rows.
  // PENNINGTON_KEYS is the authoritative source: if the study key is in the set, it's PNJ.
  // Always override based on study key — don't trust the CSV Site Name for site classification.
  ...
  // Verification: confirm all records are accounted for
  ...
  // Enhanced: severity tiers, recency, cross-study detection, category breakdown
  ...
  const riskFlags = Object.entries(cancelCounts)
    ...
    .map(([k,c]) => {
      ...
      // Cross-study: same patient in multiple studies
    ...
    // Sort: critical first, then high, then by next visit date
    .sort((a,b) => {
  ...
  const weeklyTrendsAll = (() => {
    ...
    allRows.forEach(r => {
      ...
      // Only count as cancelled if it's a true cancellation (not SF, no-show, withdrew, etc.)
    ...
    if (legacyCancels && legacyCancels.length) {
      legacyCancels.forEach(r => {
        ...
        if (!d || isExcludedStudy(r['Study Name'])) return;
        // Only count true cancellations in trends (not SF, no-show, withdrew, etc.)
...
// ═══ PATIENT STATUS TRUTH (from audit trail) ═══
// Terminal statuses = patient needs follow-up
var _TERMINAL_STATUSES = {'Screen Fail':1,'Not Interested':1,'Not Eligible':1,'Discontinued':1,'No Show/Cancelled V1':1};
// Active statuses = patient has re-engaged, no longer needs follow-up for that event
...
// ═══ ClickUp patient matching for Follow-Up table ═══
// Builds lookup maps from REFERRAL_DATA for O(1) matching by name and phone
...
function renderFollowUpTable() {
  ...
  // Build ClickUp lookup once per render
  ...
  // Build risk lookup by patient name (lowercase)
  var _riskByName = {};
  ...
  // Build audit reason lookup by patient+study (most specific reason from BQ)
  var _auditReasons = {};
  (window._subjectAudit || []).forEach(function(a) {
  ...
  // Filter out patients who have recovered (re-engaged) per audit trail
...
function buildActionSteps() {
  ...
  // Build enrolling study set from CRIO + Looker (for filtering non-enrolling insights)
  ...
  // (URGENT CALLS removed — already shown in At-Risk Patient cards with Done/N/A)
  ...
  // ═══ 7. NO SHOW FOLLOW-UP (enrolling studies only) ═══
  ...
  // ═══ 8. RETENTION (enrolling studies only) ═══════════
  ...
  // (PROACTIVE OUTREACH removed — already shown in At-Risk Patient cards with Done/N/A)
  ...
  // ── Dormant (only flag ENROLLING studies — maintenance/closed naturally wind down) ──
  if(dormEl){
...
function buildRecruitmentKPIs() {
  var el = function(id) { return document.getElementById(id); };
  // Stale leads: active referrals inactive for 30+ days (need follow-up)
...
function buildMedRecAlerts() {
  ...
  // 8. Follow-up needed: active patients not contacted in 14+ days
...
function initReferrals() {
  ...
  // Looker study status — fetch after CRIO (needs study keys for merge)
...
async function refreshReferrals() {
  ...
  try {
    ...
    // Fallback to Sheets CSV if cloud function returned empty (ClickUp API timeout)
    ...
    // Re-render studies table so Provider Leads + Total Leads columns populate
...
function renderReferralDashboard() {
  ...
  // ── Build CRIO-sourced rows FIRST (needed by KPIs and unified table) ──
  var _crioSourceRows = {};
  ...
  // (Pipeline funnel, source breakdown, study table, stale leads — removed)
  ...
  // (_crioSourceRows already built above, before KPIs)
  ...
  if (dnqEl && unifiedRows.length > 0) {
    ...
    // Table sorted by qualification rate (best sources first)
    ...
    // ── DNQ Reasons Detail (from subject audit trail) ──
    if (window._dnqReasons && window._dnqReasons.length > 0) {
      ...
      function _catReason(reason) {
        ...
        // Specific clinical reasons (most specific first)
  ...
  // ── Monthly Referral Trend by Source ──
  ...
  // Participant Status section removed (duplicative with Schedule tab)
...
function showCampaignDetailModal(studyName) {
  ...
  // CRIO Subjects for this campaign's studies (screening/enrolled from CRIO directly)
  ...
  try {
    ...
    try {
      ...
      if (rows && rows.length > 0) {
        ...
        var _bqStatusUp = {
          // Looker-correct labels (from CRIO Looker Formulas)
          ...
          // Legacy labels (backward compat with cached data)
    ...
    if (subjectsUrl) {
      try {
        ...
        if (sRows && sRows.length > 0) {
          // BQ sends mixed-case statuses — normalize to uppercase keys the dashboard expects
          var _subStatusUp = {
...
function renderCrioStudies() {
  ...
  // Set of enrolling protocol names (lowercase) — used to filter out non-enrolling study insights
  var enrollingProtocols = new Set();
  CRIO_STUDIES_DATA.forEach(function(s) {
    if (s.status === 'ENROLLING') enrollingProtocols.add(s.protocol_number.toLowerCase().trim());
  });
  // Helper: check if a CTMS study short name matches any enrolling protocol
  ...
  // 1. Win-back: V1 no-shows/cancels — only for ENROLLING studies (no value chasing non-enrolling)
  var winbacks = [];
  CRIO_STUDIES_DATA.forEach(function(s) {
    if (SKIP[s.protocol_number]) return;
  ...
  if (winbacks.length > 0) {
    ...
    winbacks.forEach(function(w) {
      ...
      var tag = w.is_prescreen ? ' <span style="font-size:8px;background:#dbeafe;color:#1843AD;padding:1px 4px;border-radius:3px;vertical-align:middle">Pre-Screen</span>' : '';
  ...
  // ── Section 1b: Recent CTMS Cancels Not Rebooked (enrolling studies only) ──
  ...
  if (qualNotSched.length > 0) {
    html += '<div style="margin-bottom:14px">'
      + '<div style="font-size:12px;font-weight:700;color:#FF9933;margin-bottom:6px">Qualified — Not Yet Scheduled for V1</div>'
  ...
  if (enrolledNoVisit.length > 0) {
    html += '<div style="margin-bottom:14px">'
      + '<div style="font-size:12px;font-weight:700;color:#072061;margin-bottom:6px">Enrolled — No Upcoming Visit Scheduled</div>'
...
// ══════════════════════════════════════════════════════════════
// CRIO Subject Detail Modal — drill down from KPI numbers
// ══════════════════════════════════════════════════════════════
...
function mergeCrioIntoStudies() {
  ...
  var STATUS_MAP = {
    // Uppercase normalized forms (Looker-correct)
    'CONFIGURING':'Configuring','STARTUP':'Startup','ENROLLING':'Enrolling',
    'MAINTENANCE':'Maintenance','PRECLOSED':'Pre-Closed','CLOSED':'Closed',
    'SUSPENDED':'Suspended','WITHDRAWN':'Withdrawn',
    // Mixed-case passthrough (Looker-correct labels from cloud function)
    'Configuring':'Configuring','Startup':'Startup','Enrolling':'Enrolling',
    'Maintenance':'Maintenance','Pre-Closed':'Pre-Closed','Closed':'Closed',
    'Suspended':'Suspended','Withdrawn':'Withdrawn',
    // Legacy labels (backward compat with cached/old data)
    'Start Up':'Startup','Closeout':'Closeout',
  ...
  CRIO_STUDIES_DATA.forEach(function(s) {
    ...
    if (existing) {
      // Merge: combine targets, subjects, revenue from second site
      ...
      // Promote status: ENROLLING wins over STARTUP/SITE QUALIFICATION/etc.
      ...
      // Keep coordinator/investigator from the enrolling site
      if (newPri > existPri) {
  ...
  DATA.enrollmentData.forEach(function(e) {
    ...
    // Use CRIO target enrollment if dashboard doesn't have one
    if (crio.target_enrollment > 0 && (!e.target || e.target === 0)) {
      e.target = crio.target_enrollment;
    }
    // Manual override targets from config (when CRIO has 0)
    ...
    // Auto-correct status from CRIO
    var crioStatus = STATUS_MAP[crio.status] || crio.status;
    if (crioStatus && e.status !== crioStatus) {
      e.status = crioStatus;
    }

    // Looker study status overrides CRIO (user-curated, authoritative)
    // Look up by primary study_key, then by extra keys (multi-site)
    ...
    // Merge subjects from all site keys for multi-site studies (e.g., PHL+PNJ)
...
function renderMedRecTab() {
  ...
  if (kpiEl) {
    ...
    kpiEl.innerHTML =
      ...
      '<div class="tb-section kpi-clickable" onclick="filterMedRec(\'pending_release\',null)"><div class="tb-label">Pending Release</div><div class="tb-value" style="color:#FF9933">'+pendRel+'</div></div>' +
      ...
      '<div class="tb-section kpi-clickable" onclick="filterMedRec(\'in_screening\',null)"><div class="tb-label">In Screening</div><div class="tb-value" style="color:#1843AD">'+inScreen+'</div></div>' +
...
function renderMetaFunnel() {
  ...
  // Unique CRM total (avoid double-counting leads matching multiple campaigns)
  if (fbStudyCol) {
    var crmSeen = new Set();
  ...
  funnelRows.forEach(function(r) {
    ...
    html += '<td style="text-align:center;color:#1843AD;">' + r.crmLeads + '</td>';
...
function renderMetaAds() {
  ...
  camps.forEach(function(c) {
    ...
    html += '<td style="text-align:center;font-weight:700;color:#1843AD;">' + c.leads.toLocaleString() + '</td>';
...
function renderMetaAlignment() {
  ...
  Object.keys(campByIndication).forEach(function(indication) {
    ...
    // Collect studies from ALL campaigns in this indication (not just first)
...
// ── 6. Enrollment Velocity Forecast ──
function renderEnrollmentVelocity() {
  ...
  // Referral pipeline counts by study (each referral counted once, first match wins)
  var refsByProto = {};
  if (REFERRAL_DATA && REFERRAL_DATA.length > 0) {
    REFERRAL_DATA.forEach(function(r) {
      var rs = (r.study || '').toLowerCase().trim();
...
async function compareBQvLegacy(bqUrl) {
  ...
  if (diffs.length) {
    html += '<details style="margin:8px 0"><summary style="cursor:pointer;font-weight:600;color:#FF9933">Field Differences (' + diffs.length + ')</summary><table style="width:100%;font-size:11px;border-collapse:collapse;margin-top:4px">';
...
function renderCoordTrendChart() {
  ...
  // Only show SCHEDULE_COORDINATORS (5 core coordinators)
...
function renderTrendsCharts() {
  ...
  // ── Chart 1: Dual-axis — Cancel Volume (bars) + Cancel Rate % (line) ──
  ...
  // ── Chart 2: Upcoming Volume per Snapshot ──
...
function renderStudiesTable() {
  ...
  // Build per-study AND per-study+site cancel breakdown counts
  ...
  function _incBreakdown(list, field) {
    (list || []).forEach(function(r) {
      var s = (r.study || '').trim();
      if (!s) return;
      // Skip patients who have recovered (re-engaged) per audit trail
      if (_patientRecovered(r.name || r.patient, s)) return;
      // Study-wide total
  ...
  const allRows = studies.map(s => {
    ...
    // Diversity badge from demographics feed
    ...
    // Per-study cancel breakdown
    // Use per-site breakdown if row has a specific site, otherwise study-wide
    ...
    // Unscheduled lookup — filter by site if row has specific site
    ...
    return `<tr style="border-bottom:1px solid var(--border);transition:background .1s" onmouseover="this.style.background='var(--surface2)'" onmouseout="this.style.background=''" data-ta="${escapeHTML(s.therapeutic_area||'')}">
      ...
      <td style="padding:10px 6px;text-align:center">${(() => {
        ...
        return '<span style="font-size:12px;font-weight:700;color:#072061;cursor:pointer" onclick="showProviderLeadsModal(\''+jsAttr(s.study)+'\')">'+provLeads.length+'</span>';
      })()}</td>
...
function generateDailyEmail(type) {
  ...
  var _ct3 = typeof window._adjCancel !== 'undefined' ? window._adjCancel : (DATA.cancelTotal||0);
  const cancelRate = DATA.upcomingTotal ? ((_ct3 / (DATA.upcomingTotal + _ct3)) * 100).toFixed(1) : '0';
  ...
  html += '<td style="padding:8px;"><div style="font-size:24px;font-weight:700;color:#dc2626;">' + _adjCancel + '</div><div style="font-size:11px;color:#64748b;">True Cancellations</div><div style="font-size:10px;color:#94a3b8">' + _adjNoShow + ' no-shows · ' + _adjWithdrew + ' withdrew · ' + _adjSF + ' SF · ' + (DATA.discontinuedTotal||0) + ' disc.</div></td>';
  ...
  html += '<td style="padding:8px;"><div style="font-size:24px;font-weight:700;color:#FF9933;">' + flags.length + '</div><div style="font-size:11px;color:#64748b;">At-Risk Patients</div></td>';
  html += '</tr></table></div>';
...
// Checks CF health endpoint and warns if data is stale
...
// Run health check 10s after load, then every 30 min
...
// Export a specific table by finding it inside a container element
...
// ═══ DATA HEALTH MONITORING (global — called by async fetch functions) ═══
...
async function _crpInit() {
  ...
  async function loadAllData() {
    ...
    if (typeof loadRegulatoryPerformance === 'function') loadRegulatoryPerformance();
