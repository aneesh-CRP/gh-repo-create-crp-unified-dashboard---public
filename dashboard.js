...
// ═══════════════════════════════════════════════════
let QB_DATA = { pnlMonthly: [], pnlByStudy: {}, qbTotalIncome: 0, invoiceLines: [], pnlClass: [], timeActivity: [], payments: [], loaded: false };
...
async function fetchQuickBooksData() {
  ...
  try {
    ...
    // ── Parse PnL Monthly — match accounts to CRIO study codes
    ...
    pnlMonthly.forEach(r => {
      ...
      if (!acct || acct.toLowerCase().startsWith('total')) return;
      ...
      const acctLower = acct.toLowerCase().replace(/[\s\-]/g, '');
      let matched = false;
      for (const code of studyCodes) {
        const codeLower = code.toLowerCase().replace(/[\s\-]/g, '');
        if (acctLower.includes(codeLower) || codeLower.includes(acctLower)) {
    ...
    QB_DATA.loaded = true;
    _log('CRP QB: Loaded — ' + Object.keys(pnlByStudy).length + ' studies matched from P&L, ' + QB_DATA.pnlClass.length + ' expense classes, total QB income: $' + Math.round(qbTotalIncome).toLocaleString());
    renderCRIOvsQB();
  } catch(e) {
...
function renderCRIOvsQB() {
  ...
  const compRows = [];
  let totalCrioRev = 0, totalQbRev = 0, matchedCount = 0, unmatchedCount = 0;
  ...
  Object.entries(studyRevMap).sort((a,b) => b[1] - a[1]).forEach(([code, crioRev]) => {
    ...
    const diff = crioRev - qbRev;
    const matchPct = crioRev > 0 && qbRev > 0 ? Math.round(qbRev / crioRev * 100) : (qbRev > 0 ? 999 : 0);

    compRows.push({
      code, sponsor, crioRev, crioAR, crioColl,
      qbRev, qbAccount: qb ? qb.qbAccount : '',
      diff, matchPct: Math.min(matchPct, 999),
      status: !qb ? 'No QB Data' :
              matchPct >= 80 && matchPct <= 120 ? 'Aligned' :
              matchPct >= 40 ? 'Partial' : 'Low Match'
    });
  });

  // Add QB-only studies (in QB P&L but not in CRIO)
  Object.entries(pnlByStudy).forEach(([code, qb]) => {
    if (!studyRevMap[code] || studyRevMap[code] <= 0) {
      compRows.push({
        code, sponsor: '', crioRev: 0, crioAR: 0, crioColl: 0,
        qbRev: qb.total, qbAccount: qb.qbAccount,
        diff: -qb.total, matchPct: 0,
        status: 'QB Only'
      });
      totalQbRev += qb.total;
    }
  ...
  const totalQbClassRev = expenseRows.reduce((s,r) => s + (parseFloat(r['Revenue'])||0), 0);

  // ── QB Payments total
  const qbTotalPay = QB_DATA.payments.reduce((s,r) => s + (parseFloat(r['Amount'])||0), 0);

  // ── KPIs
  const netProfit = totalQbClassRev - totalQbExpenses;
  const coveragePct = totalCrioRev > 0 ? Math.round(totalQbRev / totalCrioRev * 100) : 0;

  document.getElementById('qb-kpi1').textContent = fmtK(totalCrioRev);
  document.getElementById('qb-kpi1-sub').textContent = Object.keys(studyRevMap).filter(k => studyRevMap[k] > 0).length + ' active studies';
  document.getElementById('qb-kpi2').textContent = fmtK(totalQbRev);
  document.getElementById('qb-kpi2-sub').textContent = Object.keys(pnlByStudy).length + ' matched in QB P&L';
  document.getElementById('qb-kpi3').textContent = coveragePct + '%';
  document.getElementById('qb-kpi3-sub').textContent = matchedCount + ' matched, ' + unmatchedCount + ' unmatched';
  document.getElementById('qb-kpi4').textContent = fmtK(totalQbExpenses);
  document.getElementById('qb-kpi4-sub').textContent = expenseRows.length + ' classes · Payments: ' + fmtK(qbTotalPay);
  document.getElementById('qb-kpi5').textContent = fmtK(netProfit);
  document.getElementById('qb-kpi5-sub').textContent = (totalQbClassRev > 0 ? Math.round(netProfit / totalQbClassRev * 100) : 0) + '% margin';

  // ── Revenue Comparison Table
  const statusColor = { 'Aligned': '#10B981', 'Partial': '#F59E0B', 'Low Match': '#EF4444', 'No QB Data': '#9CA3AF', 'QB Only': '#60A5FA' };
  document.getElementById('qbReconBody').innerHTML = compRows.map(r =>
    '<tr>' +
    ...
    '<td class="r">' + (r.crioAR ? fmtD(r.crioAR) : '—') + '</td>' +
    '<td class="r">' + (r.crioColl ? fmtD(r.crioColl) : '—') + '</td>' +
