...
function renderCoordWorkloadBalance() {
  ...
  coordStats.forEach(function(c) {
    ...
    html += '<div style="font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">'+c.name.split(' ')[0]+'</div>';
...
function processLiveData(allRows, legacyCancels, auditLog) {
  ...
  // ── Backfill Site Name on all rows if missing from CSV ──
  // The upcoming visits CSV has no Site Name column; derive from Study Key via PENNINGTON_KEYS.
  // Cancellations CSV already has Site Name, so this is a no-op for those rows.
  [upcoming, cancels].forEach(function(arr) {
    arr.forEach(function(r) {
      if (!r['Site Name']) {
        r['Site Name'] = siteSlug(r['Study Key'], '') === PNJ ? 'Pennington, NJ' : 'Philadelphia, PA';
      }
    });
  });
