...
// ═══════════════════════════════════════════════════
let QB_DATA = { pnlMonthly: [], pnlByStudy: {}, pnlUnmatched: [], qbTotalIncome: 0, qbPeriod: '', invoiceLines: [], pnlClass: [], timeActivity: [], payments: [], empMap: {}, studyHoursAll: {}, totalPayroll: 0, totalContractors: 0, totalTrackedHours: 0, loaded: false };
...
async function fetchQuickBooksData() {
  ...
  try {
    const [pnlMonthly, pnlCls, invLines, timeAct, classes, payments] = await Promise.all([
    ...
    timeAct.filter(r => (r['Customer/Study']||'').trim()).forEach(r => {
      ...
      const study = (r['Customer/Study']||'').trim();
      const hrs = (parseFloat(r['Hours'])||0) + (parseFloat(r['Minutes'])||0)/60;
      if (!empMap[emp]) empMap[emp] = { hours: 0, studyHours: {}, entries: 0 };
    ...
    // ── Extract payroll & contractor totals from PnL expense section
    ...
    pnlMonthly.forEach(r => {
      ...
      if ((lo.includes('contractor') || lo.includes('subcontract')) && !lo.startsWith('total ')) totalContractors += tot;
    });
    ...
    _log('CRP QB: Loaded — ' + Object.keys(pnlByStudy).length + ' studies matched ($' + Math.round(matchedRev).toLocaleString() + '), ' + pnlUnmatched.length + ' unmatched QB accounts, total QB income: $' + Math.round(qbTotalIncome).toLocaleString() + ', ' + Object.keys(empMap).length + ' staff tracked (' + Math.round(totalTrackedHours) + ' hrs), payroll: $' + Math.round(totalPayroll).toLocaleString() + ', contractors: $' + Math.round(totalContractors).toLocaleString());
    renderCRIOvsQB();
  } catch(e) {
...
function renderStaffEconomics() {
  ...
  Object.entries(pnlByStudy).forEach(([code, data]) => {
    const codeBase = code.replace(/\s*\(.*$/, '').trim().toLowerCase().replace(/[\s\-]/g, '');
    qbRevLookup[data.qbAccount.toLowerCase().replace(/[\s\-]/g, '')] = data.total;
    if (codeBase.length >= 4) qbRevLookup[codeBase] = data.total;
  });
  (QB_DATA.pnlUnmatched || []).forEach(u => {
    qbRevLookup[u.account.toLowerCase().replace(/[\s\-]/g, '')] = u.total;
  });

  function getStudyRevenue(taStudy) {
    const norm = taStudy.toLowerCase().replace(/[\s\-]/g, '');
    if (qbRevLookup[norm] !== undefined) return qbRevLookup[norm];
    for (const [key, rev] of Object.entries(qbRevLookup)) {
      if (key.length >= 4 && (norm.includes(key) || key.includes(norm))) return rev;
    }
    return 0;
  }
  ...
  const empRows = Object.entries(empMap).map(([name, data]) => {
    ...
    const topStudies = studies.slice(0, 3).map(([s, h]) => {
      const short = s.length > 20 ? s.split(' ').slice(-1)[0] : s;
      return short + ' (' + Math.round(h) + 'h)';
    }).join(', ');

    // Revenue attribution: (emp hours on study / total hours on study) × study QB revenue
    let attrRevenue = 0;
    studies.forEach(([study, hrs]) => {
      const totalStudyHrs = studyHoursAll[study] || hrs;
      const studyRev = getStudyRevenue(study);
      attrRevenue += (hrs / totalStudyHrs) * studyRev;
    });

    const estCost = blendedRate * data.hours;
    const roi = estCost > 0 ? attrRevenue / estCost : 0;
    return { name, hours: data.hours, numStudies, topStudies, attrRevenue, estCost, roi, entries: data.entries };
  ...
  if (kpiEl) {
    kpiEl.innerHTML =
      '<div class="kpi"><div class="kpi-stripe" style="background:#3B82F6"></div><div class="label">Staff Tracked</div><div class="value" style="color:#3B82F6">' + empRows.length + '</div><div class="sub">' + Math.round(totalTrackedHours).toLocaleString() + ' total hours</div></div>' +
      ...
      '<div class="kpi"><div class="kpi-stripe" style="background:#10B981"></div><div class="label">Blended Rate</div><div class="value" style="color:#10B981">' + fmtD(blendedRate) + '/hr</div><div class="sub">Total cost \u00F7 tracked hours</div></div>' +
      '<div class="kpi"><div class="kpi-stripe" style="background:#F59E0B"></div><div class="label">Revenue per $1 Cost</div><div class="value" style="color:' + (avgROI >= 1 ? '#10B981' : '#EF4444') + '">' + avgROI.toFixed(2) + 'x</div><div class="sub">Attributed revenue \u00F7 est. cost</div></div>';
  }
  ...
  tbody.innerHTML = empRows.map(r => {
    ...
    return '<tr>' +
      ...
      '<td style="font-size:11px;max-width:250px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + esc(r.topStudies) + '</td>' +
      '<td class="r" style="color:#10B981;font-weight:600">' + fmtD(r.attrRevenue) + '</td>' +
      '<td class="r" style="color:#EF4444">' + fmtD(r.estCost) + '</td>' +
