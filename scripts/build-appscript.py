...
google.script.run return value limit is ~256KB, so chunks are kept under 200KB.
"""
...
# Clean up old chunk files (.html and .gs)
for old_file in glob.glob(os.path.join(APPSCRIPT_DIR, 'DashboardJS*.*')):
    os.remove(old_file)

# Write chunk files as .gs with JS content as escaped string constants.
# Uses JSON encoding to properly escape all special characters.
for i, chunk in enumerate(chunks):
    chunk_path = os.path.join(APPSCRIPT_DIR, f'DashboardJS_{i}.gs')
    # json.dumps properly escapes \, ", newlines, unicode etc.
    escaped = json.dumps(chunk)
    gs_content = f'// Auto-generated by build-appscript.py — do not edit\nvar _JS_CHUNK_{i} = {escaped};\n'
    with open(chunk_path, 'w') as f:
        f.write(gs_content)
    print(f"  DashboardJS_{i}.gs: {len(gs_content):,} bytes")
...
print(f"\nBuilt for Apps Script (async chunk-loading via .gs string constants):")
